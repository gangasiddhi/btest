 ...
 */
function ButigoAutoloader($className) {
    ...
    if (substr($className, -4) == 'Core') {
        ...
        if ($file_in_override && $file_in_classes) {
            ...
            $classInfos = new ReflectionClass($className . ((interface_exists($className, false)
                or class_exists($className, false)) ? '' : 'Core'));

            if (! $classInfos->isInterface() && substr($classInfos->name, -4) == 'Core') {
                eval(($classInfos->isAbstract() ? 'abstract ' : '') . 'class '
                    . $className . ' extends ' . $className . 'Core {}');
            }
        } elseif ($file_in_override && !$file_in_classes) {
            require_once($overrideDir . $className . '.php');
        }
...
spl_autoload_register('ButigoAutoloader');
