...
{if $products}
	...
	{foreach from=$products item=product name=i}
		<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
			...
			<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:FavlistCart('favlist_block_list', 'delete', '{$product.id_product}', {$product.id_product_attribute}, '0');" title="{l s='remove this product from my favlist' mod='blockfavourites'}"><img src="{$img_dir}icon/delete.gif" width="11" height="13" alt="{l s='Delete'}" class="icon" /></a>
		</dt>
...
{else}
	...
	{if isset($error) && $error}
		<dt>{l s='You must create a favlist before adding products' mod='blockfavourites'}</dt>
	{else}
