...
{*<script type="text/javascript">
    {literal}
	    ...
	    $("#shopping-cart-top").hover(function(){$(".check-out").css('color', '#f2018a')},
              function(){$(".check-out").css('color', '#000000')}
   //$("#shopping-cart-top")(function(){$(".check-out").css('color', '#000000')
  );
//});
    });
		/* ]]> */
...
<div id="header-info">
<div id="user-info">
	<span>	{l s='Welcome Back' mod='blockuserinfo'}&#44;&nbsp;{$customerName}</span>
			{*&#33;&nbsp;&#124;&nbsp;<a href="{$link->getPageLink('index.php')}?mylogout" title="{l s='Log out' mod='blockuserinfo'}">{l s='Log out' mod='blockuserinfo'}</a>*}
	<span id="help_desk">Bizi Arayın: 0216 418 26 26</span>
	<span>
		...
		<img src="{$img_dir}buttons/pinkarrow.png" alt="{l s='Your Shopping Cart' mod='blockuserinfo'}" />
	</span>
</div>{*end of logout*}
<div id="my-acc-cart">
	<div {if $page_name == 'identity' || $page_name == 'referrals-stylepoints' || $page_name == 'addresses' || $page_name == 'history'}class="current"{/if} id="identity-menu" onmouseover="$('div#my-acc').show()" onmouseout="$('div#my-acc').hide()">
		<a href="{*$link->getPageLink('identity.php', true)*}#" class="identity" title="{l s='HESABIM' mod='blockuserinfo'}">{l s='HESABIM' mod='blockuserinfo'}</a>
		<div id="my-acc">
			<ul>
	...
	<div id="shopping-cart-top">
		<a id ="shopping-cart-top-link" href="{$link->getPageLink('order.php')}?step=1" title="{l s='Your Shopping Cart' mod='blockuserinfo'}">
			<span class="ajax_cart_product_txt">{l s='BAG' mod='blockuserinfo'}</span>
			<span class="ajax_cart_closure">(</span>
