...
class StatsForecast extends Module
{
	...
	public function hookAdminStatsModules()
	{
		...
		$dateFromGAdd = ($cookie->stats_granularity != 42
			? 'SUBSTRING([[PREFIX]].`date_add`, 1, ' . (int)$cookie->stats_granularity . ')'
			: 'IFNULL(MAKEDATE(YEAR([[PREFIX]].`date_add`), DAYOFYEAR([[PREFIX]].`date_add`) - WEEKDAY([[PREFIX]].`date_add`)), CONCAT(YEAR([[PREFIX]].`date_add`), "-01-01*"))');

		$sql = '
			SELECT
				' . str_replace('[[PREFIX]]', 'o', $dateFromGAdd) . ' as fix_date,
			...
			GROUP BY ' . str_replace('[[PREFIX]]', 'o', $dateFromGAdd) . '
			ORDER BY fix_date';

		$result = $db->ExecuteS($sql, false);
		$result1 = $db->ExecuteS('
			SELECT ' . str_replace('[[PREFIX]]', 'o', $dateFromGAdd) . ' as fix_date,
				o.`id_order`,
				o.`total_paid`
			FROM `' . _DB_PREFIX_ . 'orders` o
			WHERE o.`valid` = 1
        ...
        foreach ($result1 as $res) {
            $total_products_array[$res['fix_date']][] = $res['total_paid'];
        }

        foreach($total_products_array as $key => $total) {
            $each_day_total_amount = 0;

            foreach($total as $total_amount) {
                $each_day_total_amount += $total_amount;
                $total_products[$key] = $each_day_total_amount;
            }
        }
		...
		while ($row = $db->nextRow($result)) {
            $row['totalProducts'] = $total_products[$row['fix_date']];
		...
		<fieldset><legend><img src="../modules/'.$this->name.'/logo.gif" /> '.$this->displayName.'</legend>
			...
			<table class="table" border="0" cellspacing="0" cellspacing="0">
				<tr>
					...
					<th style="text-align:center">'.$this->l('% Reg.').'</th>
					<th style="text-align:center">'.$this->l('% Orders').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Marketing C.').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Exchange C.').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Products Sales').'</th>
				</tr>';

		$visitArray = array();
		$sql = '
			SELECT ' . str_replace('[[PREFIX]]', 'c', $dateFromGAdd) . ' as fix_date,
				COUNT(*) as visits
			FROM `' . _DB_PREFIX_ . 'connections` c
			WHERE c.`date_add` BETWEEN ' . ModuleGraph::getDateBetween() . '
			GROUP BY ' . str_replace('[[PREFIX]]', 'c', $dateFromGAdd);
		...
		$sql = '
			SELECT ' . str_replace('[[PREFIX]]', 'o', $dateFromGAdd) . ' as fix_date,
				SUM(
			        IF(d.`id_discount_type` != 7, od.`value`, 0)
			    ) as Marketing,
			    SUM(
			        IF(d.`id_discount_type` = 7, od.`value`, 0)
			    ) as Exchange
			FROM `' . _DB_PREFIX_ . 'orders` o
			JOIN `' . _DB_PREFIX_ . 'order_discount` od ON o.`id_order` = od.`id_order`
			JOIN `bu_discount` d ON d.`id_discount` = od.`id_discount`
			WHERE o.`valid` = 1 AND o.`date_add` BETWEEN ' . ModuleGraph::getDateBetween() . '
			GROUP BY ' . str_replace('[[PREFIX]]', 'o', $dateFromGAdd);
		$discounts = Db::getInstance()->ExecuteS($sql, false);

		while ($row = $db->nextRow($discounts)) {
			$discountArray[$row['fix_date']]['Marketing'] = $row['Marketing'];
			$discountArray[$row['fix_date']]['Exchange'] = $row['Exchange'];
		}

		$today = date('Y-m-d');

		foreach ($dataTable as $row) {
			$discountTodayMarketing = (isset($discountArray[$row['fix_date']]) ? $discountArray[$row['fix_date']]['Marketing'] : 0);
			$discountTodayExchange = (isset($discountArray[$row['fix_date']]) ? $discountArray[$row['fix_date']]['Exchange'] : 0);
			...
			$row['registrations'] = Db::getInstance()->getValue('
				...
				WHERE `date_add` BETWEEN ' . ModuleGraph::getDateBetween() . ' AND `date_add` ' . $dateFromGReg
			);

			$totalHT = $row['totalProducts'];
			$this->_html .= '
			<tr>
				<td>'.$row['fix_date'].'</td>
				...
				<td align="right">' . Tools::displayPrice($discountTodayMarketing, $currency) . '</td>
				<td align="right">' . Tools::displayPrice($discountTodayExchange, $currency) . '</td>
			...
			$this->t7 += $discountTodayMarketing;
			$this->t8 += $discountTodayExchange;
			$this->t9 += $totalHT;
		}
