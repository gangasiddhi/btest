...
class PGI extends PG implements IPG, IPGBank {
    public $pgi_instal = 1;
    public $name = 'pgi';
    ...
    function __construct() {
        ...
        $this->displayName = $this->l('İŞ BANKASI POS');
        $this->description = $this->l('Payment Gateway API implementation for İş Bankası');
    ...
    function hookValidation($params) {
        require_once(dirname(__FILE__).'/lib/pgicart.php');
        require_once(dirname(__FILE__).'/lib/pgiitem.php');
        global $cookie, $cart;
        ...
        $this->log->debug("others: $this->pgi_instal \t, Total:$total\n ");

        $pgi = new PGI();
        $currency = $pgi->getCurrency();
        $product_total = floatval(number_format($cart->getOrderTotal(true, 3), 2, '.', ''));//the total of products with tax

        $this->log->debug("others: $this->pgi_instal,\t Installments: $installment_count,\t Intallment-interest: $installmnt_intrst,\t Each Install: $each_inst_amount\t Final-Total:$total\n ");

        if (_BU_ENV_ == 'production')
            $pgiCart = new PGICart( '700656449303', 'butigo', 'FZ4Dnah8M6', "production", "949", "Auth" );
        else
            $pgiCart = new PGICart( '700100000', 'mrt', 'TRps9630', "sandbox", "949", "Auth" );

        foreach ($cart->getProducts() as $product) {
            $pgiCart->AddItem(
                    new PGIItem( $product['ean13']
        ...
        $pgiCart->SetCustomerData($customer_data);

        $bankResponse = $this->getBandResponseForPayment($pgiCart);
        ...
        if($this->isBankResponseApprovedOnPayment($bankResponse)) {
            ...
            if($installment_count > 1) {
                $pgi->validateOrder(intval($cart->id), $orderState, $total, $pgi->displayName,  $installment_count,
                    $installmnt_intrst, $each_inst_amount, $data[$root]['Response']['VALUE'], array(), NULL, false, $customer->secure_key);
            } else {
                $pgi->validateOrder(intval($cart->id), $orderState, $total, $pgi->displayName,1, 0, $total,
                    $data[$root]['Response']['VALUE'], array(), NULL, false, $customer->secure_key);

            }
            $order = new Order(intval($pgi->currentOrder));
            Tools::redirectLink(Tools::getHttpHost(true, true).__PS_BASE_URI__.'order-confirmation.php?key='.$customer->secure_key.'&id_cart='.intval($cart->id).'&id_module='.intval($pgi->id).'&id_order='.intval($pgi->currentOrder));
        } else {
    ...
    function hookPayment($params)
    {
        ...
        return $this->display(__FILE__, 'pgi.tpl');
    }
        ...
        if(trim($order->module) == "pgi")
        {
            ...
            if($newOrderStatus->id == _PS_OS_REFUND_ || $newOrderStatus->id == _PS_OS_CANCELED_ || $newOrderStatus->id ==_PS_OS_PARTIALREFUND_ || $newOrderStatus->id == _PS_OS_MANUALREFUND_)
            {
                ...
                if (_BU_ENV_ == 'production')
                {
                    ...
                    $output .= "\t<Password>" .$this->xmlentities('FZ4Dnah8M6') . "</Password>\n";
                    $output .= "\t<ClientId>".$this->xmlentities('700656449303') ."</ClientId>\n";
                }
                else
                {
                    $output .= "\t<Name>".$this->xmlentities('mrt') ."</Name>\n";
                    $output .= "\t<Password>" .$this->xmlentities('TRps9630') . "</Password>\n";
                    $output .= "\t<ClientId>".$this->xmlentities('700100000') ."</ClientId>\n";
                }
                ...
                $output .= "\t<IPAddress>". Tools::getRemoteAddr() . "</IPAddress>\n";
                $output .= "</CC5Request>\n";
                ...
                $session = curl_init();
                if (_BU_ENV_ == 'production')
                    curl_setopt($session, CURLOPT_URL, "https://spos.isbank.com.tr/servlet/cc5ApiServer");
                else
