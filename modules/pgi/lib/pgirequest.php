<?php
  ...
  */
  ...
  class PGIRequest {
	 ...
	 *                         defaults to 'Auth'
     */
    function PGIRequest($id, $name, $key, $server_type="sandbox", $currency="USD", $trans_type="Auth") {
      $this->merchant_id = $id;
      ...
      require_once('pgilog.php');
      $this->log = new PGILog('', '', L_OFF);
    }

    function SetLogFiles($errorLogFile, $messageLogFile, $logLevel=L_ERR_RQST) {
      $this->log = new PGILog($errorLogFile, $messageLogFile, $logLevel);
    }
    ...
    function SendServer2ServerCart($xml_cart, $timeout=false, $die=true) {
      ...
      ini_set('include_path', ini_get('include_path').PATH_SEPARATOR.'.');
      require_once('xml/pgi_xmlparser.php');

      $xml_parser = new pgi_XmlParser($body);
