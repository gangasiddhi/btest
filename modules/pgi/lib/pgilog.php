...
class PGILog {
    ...
    // SetLogFiles
    function PGILog($errorLogFile, $messageLogFile, $logLevel=L_ERR_RQST, $die=true){
        $today = date('d-m-Y');
    	$errname = explode(".", $errorLogFile);
    	$msgname = explode(".", $messageLogFile);
    	$errorLogFile = _PS_LOG_DIR_.'/pgi/'.$errname[0].'_'.$today.'.'.$errname[1];
    	$messageLogFile = _PS_LOG_DIR_.'/pgi/'.$msgname[0].'_'.$today.'.'.$msgname[1];
