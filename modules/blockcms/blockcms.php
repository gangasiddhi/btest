...
class BlockCms extends Module
{
	...
	public function install()
	{
		...
		if (!parent::install() OR !$this->registerHook('top') OR!$this->registerHook('leftColumn') OR !$this->registerHook('rightColumn') OR !$this->registerHook('footer') OR !$this->registerHook('header') OR
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('submitBlockCMS')) {
			...
			if (Tools::isSubmit('addBlockCMS')) {
				...
				$languageQuery = 'INSERT INTO `'._DB_PREFIX_.'cms_block_lang` (`id_cms_block`, `id_lang`, `name`)
					VALUES';
				foreach ($languages as $language) {
					$languageQuery .= '('.(int)$id_cms_block.', '.(int)$language['id_lang'].',
									"'.pSQL(Tools::getValue('block_name_'.$language['id_lang'])).'"),';
				}
				$languageQuery = rtrim($languageQuery,',');
				Db::getInstance()->Execute($languageQuery);
			} elseif (Tools::isSubmit('editBlockCMS')) {
			...
			$cmsBoxes = Tools::getValue('cmsBox');
			if (sizeof($cmsBoxes)) {
				$query = 'INSERT INTO `'._DB_PREFIX_.'cms_block_page` (`id_cms_block`, `id_cms`, `is_category`) VALUES';
				foreach ($cmsBoxes as $cmsBox) {
					$cms_properties = explode('_', $cmsBox);
					$query .= '('.(int)$id_cms_block.', '.(int)$cms_properties[1].', '.(int)$cms_properties[0].'),';
				}
				$query = rtrim($query, ',');
