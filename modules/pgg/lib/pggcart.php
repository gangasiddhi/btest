 ...
 */
class PGGCart implements IPGCart {
    ...
    var $server_url = "https://sanalposprov.garanti.com.tr/VPServlet";
    var $request_url;
    ...
    function PGGCart($mode, $version, $terminal_id, $prov_user_id, $merchant_id, $prov_password, $currency, $trans_type, $host) {
     ...
     *
     * @param PGGItem $item an object that represents an item
     *
     * @return void
     */
     ...
     */
    function GetXML() {
        require_once('xml/pgg_xmlbuilder.php');

        $this->hash_data = strtoupper(
            sha1(
                $this->customer_arr['OrderId']
                . $this->terminal_id
                . $this->customer_arr['Number']
                . $this->customer_arr['Total']
                . $this->security_data
            )
        );
        $xml_data = new pgg_XmlBuilder();
     ...
     * Creates a PGGRequest object (defined in pggrequest.php) and sends it to the server.
     *
    ...
    function CheckoutServer2Server($timeout=false, $die=true) {
        ...
        require_once(dirname(__FILE__) . '/pggrequest.php');

        $PGGRequest = new PGGRequest(
            $this->merchant_id,
            $this->merchant_name,
            $this->merchant_key,
            $this->server_url,
        ...
        $PGGRequest->SetLogFiles('pggerror.log', 'pggmessage.log', L_ALL);

        return $PGGRequest->SendServer2ServerCart($this->GetXML(), $timeout, $die);
    }

    /**
     * TODO: not using anywhere. Remove
     * @access private
     */
    /*
    ...
    */
    ...
    /**
     * TODO: remove. Not using anywhere
    ...
    // TODO: remove. Not using anywhere
    protected function isChildOf($path = '', $parents = array()) {
