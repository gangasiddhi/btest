<?php
  ...
  */
  ...
  class PGGRequest {
	 ...
	 *                         defaults to 'Auth'
     */
    function PGGRequest($id, $name, $key, $host, $currency="USD", $trans_type="Auth") {
      $this->merchant_id = $id;
      ...
      require_once('pgglog.php');

      $this->log = new PGGLog('pggerror.log', 'pggmessage.log', L_OFF);
    }

    function SetLogFiles($errorLogFile, $messageLogFile, $logLevel=L_ERR_RQST) {
      $this->log = new PGGLog($errorLogFile, $messageLogFile, $logLevel);
    }
    ...
    function SendServer2ServerCart($xml_cart, $timeout=false, $die=true) {
      ...
      ini_set('include_path', ini_get('include_path').PATH_SEPARATOR.'.');
      require_once('xml/pgg_xmlparser.php');

      $xml_parser = new pgg_XmlParser($body);
 ...
 */
	...
	if ($log)
	{
		$myFile = _PS_LOG_DIR_."/pgg_url.txt";
