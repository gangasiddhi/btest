...
class PGGLog {
    ...
    // SetLogFiles
    function PGGLog($errorLogFile, $messageLogFile, $logLevel = L_ERR_RQST, $die = true) {
        $today = date('d-m-Y');
        $errname = explode(".", $errorLogFile);
        $msgname = explode(".", $messageLogFile);
        $errorLogFile = _PS_LOG_DIR_ . '/pgg/' . $errname[0] . '_' . $today . '.' . $errname[1];
        $messageLogFile = _PS_LOG_DIR_ . '/pgg/' . $msgname[0] . '_' . $today . '.' . $msgname[1];
        $this->logLevel = $logLevel;

        if (! $this->errorLogFile = @fopen($errorLogFile, "a") OR ! $this->messageLogFile = @fopen($messageLogFile, "a")) {
            $log = "Log files are not writable, set them to 777";
