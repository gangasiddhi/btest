...
function rating_bar($id, $units='', $static='')
{
	...
	if ($static == 'static')
	{
		...
		$static_rater[] .= '</ul>';
		$static_rater[] .= '<p>'.$productrating->traduz( 'Rating' ).': <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' '.$productrating->traduz( 'cast' ).') </p>';
		$static_rater[] .= '</div>';
		$static_rater[] .= '</div>'."\n\n";
	...
	else
	{
		...
		for ($ncount = 1; $ncount <= $units; $ncount++)
		{
			...
			if(!$voted)
			{
				...
				$rater.='<li><a href="'.$rating_path_db.'?j='.$ncount.'&amp;q='.$id.'&amp;t='.$ip.'&amp;c='.$units.'" title="'.$productrating->traduz('Give your rating now').': '.$ncount.' '.$productrating->traduz('out of').' '.$units.'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>';
			}
		}
		$ncount=0; // resets the count

		$rater.='  </ul>';
		$rater.='  <p itemprop="review" itemscope itemtype="http://data-vocabulary.org/Review-aggregate"';
		if ($voted)
		{
			$rater .= ' class="voted"';
		}
		$rater.='>'.$productrating->traduz('Rating').': <strong itemprop="rating"> '.$rating1.'</strong>/'.$units.' (<span itemprop="count">'.$count.'</span> '.$tense.' '.$productrating->traduz('cast').')';
		$rater.='  </p>';
		$rater.='</div>';
		$rater.='</div>';
