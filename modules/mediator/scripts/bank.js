...
function popupInstallment() {
    ...
    $.each(installmentsOptions, function(k, v) {
        var pop_div = '';
        var bank = baseDir + 'modules/mediator/img/' + k + '.png';
        ...
        $.each(v, function(i, r) {
            ...
            pop_div += '<tr class="helight"><td style="padding:0 8px 0 0;">' + (i != 1 ? installmentOptionName : one_payment) +
                '</td><td style="padding:0 7px 0 0;">' + installmentsAmount[k]['eachInstallmentAmountFor' + i + 'Installments'] +
                ' ' + sign + '</td><td>' + installmentsAmount[k]['totalAmountFor' + i + 'Installments'] +
                ' ' + sign + '</td></tr>';

            pop_div += '<tr class="spacer"><td></td><td></td><td></td></tr>';
        });

        pop_install += '<div class="pop-bank-instal">';
        pop_install += '<table '+(i == 2 ? 'style="width: 320px;"': '')+'><thead><tr><td colspan="3"><img src="' + bank + '" alt=""/></td></tr></thead>';
        pop_install += pop_div;
        pop_install += '</table>';
        pop_install += '</div>';
...
$(document).ready(function() {
    ...
    $('#pay_button').click(function(e) {
        ...
        if (credit_card_no === '' || cc_exp_year === '00' || cc_exp_month === '00' || ccvv2 === '') {
            alert(missing_field_error);

            return false;
        }

        if (! $('input[name="check_agree"]:checked').val()) {
            alert(error_msg);
