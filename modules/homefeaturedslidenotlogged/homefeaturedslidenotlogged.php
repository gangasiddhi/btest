...
class HomeFeaturedSlideNotLogged extends Module
{
	/** @var max image size */
	...
	protected $_xml;
 	...
 	{
 	 	$this->name = 'homefeaturedslidenotlogged';
		$this->tab = 'front_office_features';
 	 	$this->version = '1.4';
		/* The parent construct is required for translations */
	 	parent::__construct();
		$this->page = basename(__FILE__, '.php');
	 	$this->displayName = $this->l('Home Page Featured Slide Show - Customer Logged Out');
    ...
    function install()
    {
        if (!parent::install() OR !$this->registerHook('HomeLoggedOut') OR !$this->registerHook('header'))
            return false;
        return true;
    }
			...
			foreach ($_POST['link'] as $link)
			{
				...
				if (isset($_FILES['link_'.$i.'_img']) AND isset($_FILES['link_'.$i.'_img']['tmp_name']) AND !empty($_FILES['link_'.$i.'_img']['tmp_name']))
				{
					...
					if ($error = checkImage($_FILES['link_'.$i.'_img'], $this->maxImageSize))
						$this->_html .= $error;
					elseif(!move_uploaded_file($_FILES['link_'.$i.'_img']['tmp_name'], dirname(__FILE__).'/slides/bg_img_0'.$i.'.jpg'))
						return false;
//				elseif (!$tmpName = tempnam(_PS_TMP_IMG_DIR_, 'PS') OR !move_uploaded_file($_FILES['link_'.$i.'_img']['tmp_name'], $tmpName))
//					return false;
//                elseif (!imageResize($tmpName, dirname(__FILE__).'/slides/bg_img_0'.$i.'.jpg'))
//					$this->_html .= $this->displayError($this->l('An error occurred during the image upload.'));
//                unlink($tmpName);
				}
				if ($line = $this->putContent($newXml, 'img', 'slides/bg_img_0'.$i.'.jpg'))
					$newXml .= $line;
				$newXml .= "\n".'	</link>'."\n";
				$i++;
            }
            ...
            $cdnParams = array("uris" => array());

            for($ii = 0; $ii < $i; $ii++) {
                $imgLink = $this->_path . 'slides/bg_img_0'.$ii.'.jpg';
                $cdnParams['uris'][] = $imgLink;
            }

            Module::hookExec('invalidateCDN', $cdnParams); 
	...
	public function _getFormItem($i, $last)
	{
		...
		$output .= '
				...
				<div class="margin-form">
					<div><img src="'.$this->_path.'slides/bg_img_0'.$i.'.jpg" alt="" title="" style="width:250px; height:auto;" /></div>
	...
	function hookHomeLoggedOut($params)
 	{
        ...
        /* Languages preliminaries */
 	 	/*if (file_exists(dirname(__FILE__).'/links.xml'))
 	 		if ($xml = simplexml_load_file(dirname(__FILE__).'/links.xml'))
 	 		{*/
            $bgi_num=Tools::getValue('bgi', -1);
            if( $bgi_num >= 1 && $bgi_num <= 7 ) {
                if($bgi_num == 7)
                   $smarty->assign('ivana_img', 1);
                $smarty->assign(array(
                    'this_path' => $this->_path,
                    'bg_image' => $this->_path.'slides/bg_img_0'.($bgi_num -1).'.jpg'
                ));
            } else {
                $show_fb_form = Tools::getValue('fb', -1);
                if($show_fb_form !== -1) {
                    $smarty->assign(array(
                        'str' => 'name,email,password', /*Fields required in Facebook Registration Form*/
                        'this_path' => $this->_path,
                        'bg_image' => $this->_path.'slides/bg_img_06.jpg'
                    ));
                    $smarty->assign('show_fb_form', 1);
                     //return $this->display(__FILE__, 'homefeaturedslidenotloggedfb.tpl');
                } else {
                    $images = array(0,1,2,3,4,5,6);
                    $image_num = array_rand($images);
                    if($image_num == 6)
                        $smarty->assign('ivana_img', 1);

                    $smarty->assign(array(
                        'this_path' => $this->_path,
                        'bg_image' => $this->_path.'slides/bg_img_0'.$image_num.'.jpg'
                    ));
                }
            }
            return $this->display(__FILE__, 'homefeaturedslidenotlogged.tpl');
 	}
	...
	public function hookHeader($params)
	{
		...
		if(strpos($_SERVER['PHP_SELF'], 'logged-out')!== false && !$cookie->isLogged())
			Tools::addCSS($this->_path.'assets/homefeaturednotlogged.css', 'all');
	}
}
?>
