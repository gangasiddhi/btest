...
require_once(dirname(__FILE__) . '/pgtwlog.php');
...
class PGTWRequest {
     ...
     * @param string $server_url the server url that the request will be sent
     ...
     *        defaults to 'Auth'
     */
    function PGTWRequest($id, $name, $key, $server_url, $currency = "USD", $trans_type = "Auth") {
        $this->merchant_id = $id;
    ...
    function SendServer2ServerCart($xml_cart, $timeout = false, $die = true) {
        list($status, $body) = $this->SendReq($this->server_url, $this->GetAuthenticationHeaders(), $xml_cart, $timeout);

        if (! empty($this->log)) {
            $this->log->LogRequest($body);
        }

        ini_set('include_path', ini_get('include_path') . PATH_SEPARATOR . '.');
        require_once('xml/pgtw_xmlparser.php');

        $xml_parser = new pgtw_XmlParser($body);
    ...
    function SendReq($url, $header_arr, $postargs, $timeout=false) {
        ...
        if ($log) {
            $myFile = _PS_LOG_DIR_."/pgtw_url.txt";
