...
if (! defined('L_OFF')) {
    ...
    define("L_RESP", 4); // Log Response To Google
...
class PGTWLog {
    ...
    // SetLogFiles
    function PGTWLog($errorLogFile, $messageLogFile, $logLevel = L_ERR_RQST, $die = true) {
        $today = date('d-m-Y');
        $errname = explode(".", $errorLogFile);
        $msgname = explode(".", $messageLogFile);
        $errorLogFile = _PS_LOG_DIR_ . '/pgtw/' . $errname[0] . '_' . $today . '.' . $errname[1];
        $messageLogFile = _PS_LOG_DIR_ . '/pgtw/' . $msgname[0] . '_' . $today . '.' . $msgname[1];
