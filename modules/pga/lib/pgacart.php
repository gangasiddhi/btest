...
class PGACart implements IPGCart {
    var $merchant_id;
    var $merchant_name;
    var $merchant_key;
    var $trans_type;
    ...
    /**
     * Has all the logic to build the cart's xml (or html) request to be posted to PGA servers.
     *
     ...
     *        defaults to 'Auth'
     */
    function PGACart($id, $name, $key, $server_type = "sandbox", $currency = "USD", $trans_type = "Auth") {
        $this->merchant_id = $id;
     ...
     *
     * @param PGAItem $item an object that represents an item
     *
     * @return void
     */
     ...
     */
    function GetXML() {
        require_once('xml/pga_xmlbuilder.php');

        $xml_data = new pga_XmlBuilder();
     ...
     * Creates a PGARequest object (defined in pgarequest.php) and sends it to the server.
     *
    ...
    function CheckoutServer2Server($timeout = false, $die = true) {
        ...
        require_once(dirname(__FILE__).'/pgarequest.php');

        $PGARequest = new PGARequest(
            $this->merchant_id,
            $this->merchant_name,
            $this->merchant_key,
            $this->server_url == "https://www.sanalakpos.com/servlet/cc5ApiServer" ? "production" : "sandbox",
            $this->currency,
            $this->trans_type
        );
        $PGARequest->SetLogFiles('pgaerror.log', 'pgamessage.log', L_ALL);

        return $PGARequest->SendServer2ServerCart($this->GetXML(), $timeout, $die);
    }
