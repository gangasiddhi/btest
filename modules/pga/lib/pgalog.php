...
class PGALog {
    ...
    // SetLogFiles
    function PGALog($errorLogFile = 'pgaerror.log', $messageLogFile = 'pgamessage.log', $logLevel=L_ALL, $die=true){
        $today = date('d-m-Y');
    	$errname = explode(".", $errorLogFile);
    	$msgname = explode(".", $messageLogFile);
    	$errorLogFile = _PS_LOG_DIR_.'/pga/'.$errname[0].'_'.$today.'.'.$errname[1];
    	$messageLogFile = _PS_LOG_DIR_.'/pga/'.$msgname[0].'_'.$today.'.'.$msgname[1];
