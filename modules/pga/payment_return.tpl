...
<div class="shopping-cart">
    ...
    <div id="order-detail-content" class="table_block">
        <table id="order_summary">
            <thead>
                <tr>
                    <th class="order_product item">{l s='Product' mod='pga'}</th>
                    <th class="order_quantity item">{l s='Qty' mod='pga'}</th>
                    <th class="order_unit item">{l s='Unit price' mod='pga'}</th>
                    <th class="order_total last_item">{l s='Total' mod='pga'}</th>
                </tr>
            </thead>
            ...
            <tfoot>
                <tr class="order_total_price">
                    <td colspan="3" class="order_total_label">
                        {l s='Total' mod='pga'}
                        &nbsp;
                        <span>({l s='Tax Incl.' mod='pga'})</span>
                    </td>
            ...
            <tbody>
                            ...
                            <tr class="alternate_item cart_item">
                                ...
                                <td class="cart_quantity">
                                    <a class="cart_quantity_delete"
                                        ...
                                        <img src="{$img_dir}icon/delete.gif" alt="{l s='Delete' mod='pga'}" title="{l s='Delete this customization' mod='pga'}"
                                    ...
                                    <a class="cart_quantity_up" href="{$base_dir_ssl}cart.php?add&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$token_cart}"
                                        title="{l s='Add' mod='pga'}">
                                        <img src="{$img_dir}icon/quantity_up.gif" alt="{l s='Add' mod='pga'}" width="14" height="9" />
                                    </a>
                                    ...
                                    <a class="cart_quantity_down" href="{$base_dir_ssl}cart.php?add&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;op=down&amp;token={$token_cart}"
                                        title="{l s='Subtract' mod='pga'}">
                                        <img src="{$img_dir}icon/quantity_down.gif" alt="{l s='Subtract' mod='pga'}" width="14" height="9" />
                                    </a>
                                </td>
                ...
                {if $order->total_shipping != 0}
                    <tr class="shipping_cost">
                        <td colspan="3" class="shipping-label">{l s='Shipping Cost' mod='pga'}</td>
                ...
                {if $order->installment_count > 1}
                    <tr class="installment-details">
                        <td colspan="3" class="installment-label">{l s='Interest Amount' mod='pga'}&nbsp;({$order->installment_count}&nbsp;{l s='Installments' mod='pga'})</td>
                ...
                {if $discounts}
                    {foreach from=$discounts item=discount name=discountLoop}
                        <tr class="order_discount {if $smarty.foreach.discountLoop.last}last_item{elseif $smarty.foreach.discountLoop.first}first_item{else}item{/if}">
                            <td class="discount-description" colspan="3">{l s='Discount' mod='pga'}&nbsp;({$discount.name})</td>
                            <td class="order_total">
                                <span class="price">
                                    {if $discount.value > 0}
                                        {if ! $priceDisplay}
