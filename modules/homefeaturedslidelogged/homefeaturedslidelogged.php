...
class HomeFeaturedSlideLogged extends Module
{
	/** @var max image size */
	...
	protected $_xml;
 	...
 	{
 	 	$this->name = 'homefeaturedslidelogged';
		$this->tab = 'front_office_features';
 	 	$this->version = '1.4';
		/* The parent construct is required for translations */
	 	parent::__construct();
		$this->page = basename(__FILE__, '.php');
	 	$this->displayName = $this->l('Home Page Featured Slide Show - Customer Logged In');
	 	$this->description = $this->l('Slide the images with choosen languages');
		$this->_xml = $this->_getXml();
 	}
    function install()
    {
        if (!parent::install() OR !$this->registerHook('HomeLoggedIn') OR !$this->registerHook('header'))
            return false;
        return true;
    }
...
//                unlink($tmpName);
            ...
            for($ii = 0; $ii < $i; $ii++) {
                $imgLink = $this->_path . 'slides/slide_0'.$ii.'.jpg';
	...
	function hookHomeLoggedIn($params)
 	{
        global $cookie;
        /* Languages preliminaries */
        $defaultLanguage = intval(Configuration::get('PS_LANG_DEFAULT'));
 	 	...
 	 	if (file_exists(dirname(__FILE__).'/links.xml'))
 	 		if ($xml = simplexml_load_file(dirname(__FILE__).'/links.xml'))
 	 		{
				...
				$smarty->assign(array(
					...
					'no_of_imgs' => count($xml)
				));
				return $this->display(__FILE__, 'homefeaturedslidelogged.tpl');
			}
		return false;
 	}
...
//		Tools::addJS(($this->_path).'js/jquery.nivo.slider.pack.js', 'all');
//
//	}
	...
	public function hookHeader($params)
	{
		...
		if(strpos($_SERVER['PHP_SELF'], 'index')!== false && $cookie->isLogged())
		{
			...
			Tools::addCSS($this->_path.'assets/homefeaturedlogged.css', 'all');
		}
	}
}
?>
