...
class ReferralProgramModule extends ObjectModel
{
	...
	protected $fieldsRequired = array('id_sponsor', 'email');
	...
	public function getFields()
	{
		...
		$fields['refer_type'] = (int)$this->refer_type;
		return $fields;
	}
	...
	/**
	  ...
	  */
	public static function getSponsorFriend($id_customer, $restriction = false, $pagination = false, $limit = 10, $page = 1) {
		if (!(int)($id_customer)) {
			return array();
		}

		$query = 'SELECT '.($pagination ? 'SQL_CALC_FOUND_ROWS ' : '').' s.*
		...
		if ($pagination) {
			$query .= ' LIMIT '.(((int)($page) - 1) * (int)($limit)).', '.(int)($limit);
		}
	...
	/**
	  * Return if an email and idReferralProgram is already registered
	  * based on the sponser id
	  */
	static public function getRefferalEmails($id_customer)
	{
         $result = Db::getInstance()->ExecuteS('
		    SELECT email AS email,id_referralprogram AS refer_id
			FROM `'._DB_PREFIX_.'referralprogram`
			WHERE `id_sponsor`	= '.intval($id_customer));
		 return $result;
	}
	...
	public static function sponsorDetails($month)
	{
		$result = Db::getInstance()->ExecuteS('
		    SELECT rp.`id_sponsor` AS id_sponsor,COUNT( rp.`id_sponsor` ) AS sponsorcount,rp.refer_type,c.`email` as email,c.firstname,c.lastname
			FROM `'._DB_PREFIX_.'referralprogram` rp, `'._DB_PREFIX_.'customer` c
			WHERE rp.`id_customer` != 0 AND rp.`id_sponsor` = c.`id_customer` AND MONTH(rp.`date_upd`)='.$month.'
			GROUP BY rp.`id_sponsor`
			ORDER BY sponsorcount DESC LIMIT 0, 10 ');
		 return $result;
	}
	public static function sponsorDetailsCount($id_spons,$refer_val,$month)
	{
		$result = Db::getInstance()->getRow('
		    SELECT COUNT( rp.`id_sponsor` ) AS individual_count
			FROM `'._DB_PREFIX_.'referralprogram` rp
			WHERE rp.`id_customer` != 0 AND rp.`refer_type` = '.intval($refer_val).' AND rp.`id_sponsor` ='.$id_spons.' AND MONTH(rp.`date_upd`)='.$month );
		 return $result;
	}
}
