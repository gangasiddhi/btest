...
if (Tools::isSubmit('paymentSubmit'))
{
	...
	if(isset($_POST['pgf_instl']))
		$pgf_instl = Tools::getValue('pgf_instl');
	else 
		$pgf_instl = 1;
	$log = true;
	...
	if ($log)
	{
		$myFile = _PS_LOG_DIR_."/pgf-total.txt";
		$fh = fopen($myFile, 'w') or die("can't open file");
		fwrite($fh, "install: $pgf_instl,\n".Tools::getValue('pgf_instl'));
		fclose($fh);
	}
	...
	$pgf = new PGF();
	$pgf->hookValidation(array('card_name' => $cardName, 'card_number' => $cardNumber, 'card_expiry' => $cardExpiry, 'card_cvv' => $cardCVV2, 'pgf_instl'=>$pgf_instl));
}
else
{
...
//$order = new Order($pgf->currentOrder);
//$pgf->storeCard($order->id, $cardName, $cardNumber);

//Tools::redirectLink(__PS_BASE_URI__.'order-confirmation.php?id_cart='.$cart->id.'&id_module='.$pgf->id.'&id_order='.$pgf->currentOrder.'&key='.$order->secure_key);
