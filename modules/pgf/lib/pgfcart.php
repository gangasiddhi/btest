 ...
 */
class PGFCart implements IPGCart {
    var $merchant_id;
    var $merchant_name;
    var $merchant_key;
    var $trans_type;
    ...
    /**
     * Has all the logic to build the cart's xml (or html) request to be posted to PGF servers.
     *
     ...
     *        defaults to 'Auth'
     */
    function PGFCart($id, $name, $key, $server_type = "sandbox", $currency = "USD", $trans_type = "Auth") {
        $this->merchant_id = $id;
        ...
        if(strtolower($server_type) == "sandbox") {
            ...
            $this->server_url = "https://www.fbwebpos.com/servlet/cc5ApiServer";
        }
     ...
     *
     * @param PGFItem $item an object that represents an item
     *
     * @return void
     */
     ...
     */
    function GetXML() {
        require_once('xml/pgf_xmlbuilder.php');

        $xml_data = new pgf_XmlBuilder();
     ...
     * Creates a PGFRequest object (defined in pgfrequest.php) and sends it to the server.
     *
    ...
    function CheckoutServer2Server($timeout = false, $die = true) {
        ...
        require_once(dirname(__FILE__).'/pgfrequest.php');

        $PGFRequest = new PGFRequest(
            $this->merchant_id,
            $this->merchant_name,
            $this->merchant_key,
            $this->server_url=="https://www.fbwebpos.com/servlet/cc5ApiServer" ? "production" : "sandbox",
            $this->currency,
            $this->trans_type
        );
        $PGFRequest->SetLogFiles('pgferror.log', 'pgfmessage.log', L_ALL);

        return $PGFRequest->SendServer2ServerCart($this->GetXML(), $timeout, $die);
    }
