<?php
  ...
  */
  ...
  class PGFRequest {
	 ...
	 *                         defaults to 'Auth'
     */
    function PGFRequest($id, $name, $key, $server_type="sandbox", $currency="USD", $trans_type="Auth") {
      $this->merchant_id = $id;
      ...
      require_once('pgflog.php');
      $this->log = new PGFLog('', '', L_OFF);
    }

    function SetLogFiles($errorLogFile, $messageLogFile, $logLevel=L_ERR_RQST) {
      $this->log = new PGFLog($errorLogFile, $messageLogFile, $logLevel);
    }
    ...
    function SendServer2ServerCart($xml_cart, $timeout=false, $die=true) {
      ...
      ini_set('include_path', ini_get('include_path').PATH_SEPARATOR.'.');
      require_once('xml/pgf_xmlparser.php');

      $xml_parser = new pgf_XmlParser($body);
