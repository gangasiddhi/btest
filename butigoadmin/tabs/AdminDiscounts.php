...
class AdminDiscounts extends AdminTab
{
	public function __construct()
	{
		...
		IF(a.id_discount_type IN (2, 7), CONCAT(a.value, " ", c.sign),
		"--")) as strvalue';
		...
		'PS_BUY1_GET1_FREE_VOUCHERS' => array('title' => $this->l('Enable Buy1 Get1 Free vouchers:'), 'desc' => $this->l('Allow the use of Buy1 Get1 Free vouchers in shop'), 'cast' => 'intval', 'type' => 'bool'),
		'BU_CATEGORY_DISCOUNT_ENABLE' => array('title'=> $this->l('Enable Category Discount:'), 'desc' => $this->l('Allow the use of category discount'),'cast' => 'intval', 'type' => 'bool')
		);
		parent::__construct();
	}
	...
	public function displayForm($isMainTab = true)
	{
		...
		<script type="text/javascript">
			function discountType()
			{
				...
				else if ($("#id_discount_type").val() == 2 || $("#id_discount_type").val() == 7)
				{
					$("#value-div").css("display", "block");
				...
				else if ($("#id_discount_type").val() == 6)
				{
					$("#value-div").css("display", "block");
					$("#percent-span").css("display", "none");
					$("#id_currency").css("display", "block");
					...
					$("#value-div").css("display", "none");
				else if ($("#id_discount_type").val() == 4)
				{
					$("#value-div").css("display", "block");
					$("#percent-span").css("display", "none");
					$("#id_currency").css("display", "block");
				...
				if ($(\'#id_discount_type\').val() != 2 && $(\'#id_discount_type\').val() != 7)
					$(\'#behavior_not_exhausted\').hide();
		...
		echo '
							...
							<p style="padding:0px; margin:0px 0px 10px 0px;display:none;">'.$this->l('Mark all checkbox(es) of categories to which the discount is to be applied').'<sup> *</sup></p>
						</div>
				<div class="clear" / >
