...
class AdminPerformance extends AdminTab
{
	public function postProcess()
	{
		...
		{
            $this->_invalidateConfigurationCache();
		...
		if (Tools::isSubmit('submitCCC'))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				...
				if (
					...
					!Configuration::updateValue('PS_HIGH_HTML_THEME_COMPRESSION', (int)Tools::getValue('PS_HIGH_HTML_THEME_COMPRESSION')) OR
					!Configuration::updateValue('PS_CSS_JS_VERSION', Tools::getValue('PS_CSS_JS_VERSION'))
				)
		...
		if (Tools::isSubmit('submitCacheInvalidation'))
		{
			if ($this->tabAccess['edit'] === '1')
			{
                $this->_invalidateConfigurationCache();
		...
		if (Tools::isSubmit('submitCacheConfig'))
		{
			if ($this->tabAccess['edit'] === '1')
			{
                $this->_invalidateConfigurationCache();

				Configuration::updateValue('PS_BCACHE_MEMCACHED', Tools::getValue('enable_bcache_memcached', 0));
				Configuration::updateValue('PS_BCACHE_LOG_GET', Tools::getValue('enable_bcache_log_get', 0));
				Configuration::updateValue('PS_BCACHE_LOG_SET', Tools::getValue('enable_bcache_log_set', 0));
				Configuration::updateValue('PS_BCACHE_LOG_DELETE', Tools::getValue('enable_bcache_log_delete', 0));
				Tools::redirectAdmin($currentIndex.'&token='.Tools::getValue('token').'&conf=4');
			}
			else
				$this->_errors[] = Tools::displayError('You do not have permission to edit here.');
    ...
    private function _invalidateConfigurationCache() 
    {
        BCache::remove('butigoConfiguration');
        BCache::remove('butigoLanguages');
    }
	...
	public function display()
	{
		...
		<form action="'.$currentIndex.'&token='.Tools::getValue('token').'" method="post" style="margin-top:10px;">
			<fieldset>
				<legend><img src="../img/admin/prefs.gif" /> '.$this->l('Configuration Cache').'</legend>

				<label>'.$this->l('BCache Memcahe Enabled?:').'</label>
				<div class="margin-form">
					<input type="radio" name="enable_bcache_memcached" id="enable_memcached_over_bcache" value="1" '.(Configuration::get('PS_BCACHE_MEMCACHED') ? 'checked="checked"' : '').' /> <label class="t"><img src="../img/admin/enabled.gif" alt="" /> '.$this->l('Yes').'</label>
					<input type="radio" name="enable_bcache_memcached" id="enable_memcached_over_bcache_2" value="0" '.(!Configuration::get('PS_BCACHE_MEMCACHED') ? 'checked="checked"' : '').' /> <label class="t"><img src="../img/admin/disabled.gif" alt="" /> '.$this->l('No').'</label>
					<p>'.$this->l('This enables memcaced used by BCache class').'</p>
				</div>

				<label>'.$this->l('Log BCache events:').'</label>
				<div class="margin-form">
					<input type="checkbox"" name="enable_bcache_log_get" id="enable_bcache_log_get" value="1" '.(Configuration::get('PS_BCACHE_LOG_GET') ? 'checked="checked"' : '').' /> <label class="t"><img src="../img/admin/enabled.gif" alt="" /> '.$this->l('GET').'</label>
					<input type="checkbox"" name="enable_bcache_log_set" id="enable_bcache_log_set" value="1" '.(Configuration::get('PS_BCACHE_LOG_SET') ? 'checked="checked"' : '').' /> <label class="t"><img src="../img/admin/enabled.gif" alt="" /> '.$this->l('SET').'</label>
					<input type="checkbox"" name="enable_bcache_log_delete" id="enable_bcache_log_delete" value="1" '.(Configuration::get('PS_BCACHE_LOG_DELETE') ? 'checked="checked"' : '').' /> <label class="t"><img src="../img/admin/enabled.gif" alt="" /> '.$this->l('DELETE').'</label>
					<p>'.$this->l('This enables memcaced used by BCache class').'</p>
				</div>
				...
				<div class="margin-form">
					<input type="submit" value="'.$this->l('   Save Changes   ').'" name="submitCacheConfig" class="button" />
					<input type="submit" value="'.$this->l('   Invalidate Configuration Cache   ').'" name="submitCacheInvalidation" class="button" />
				</div>
			</fieldset>
		</form>';
		...
		<form action="'.$currentIndex.'&token='.Tools::getValue('token').'" method="post" style="margin-top:10px;">
			<fieldset>
				...
				<label>'.$this->l('Compressed CSS and JS version').' </label>
				<div class="margin-form">
					<input type="text" name="PS_CSS_JS_VERSION" id="PS_CSS_JS_VERSION" value="'.Tools::getValue('PS_CSS_JS_VERSION', Configuration::get('PS_CSS_JS_VERSION')).'" size="10" />
					<p>'.$this->l('Change this to the next version whenever there is any change in any CSS or JS').'</p>
				</div>
