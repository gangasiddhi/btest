...
class AdminProducts extends AdminTab
{
	...
	public function __construct()
	{
		...
		$this->fieldsDisplay = array(
			...
			'wholesale_price' => array('title' => $this->l('Wholesale Price'),'price' => true,'align' => 'center', 'width' => 45),
			...
			'showing_status' => array('title' => $this->l('Showing Status'), 'active' => 'showstatus', 'align' => 'center', 'type' => 'bool', 'orderby' => false)
		);
...
//		echo Tools::getValue('id_product_type');exit;
		$object->id_product_type = Tools::getValue('id_product_type');
        $object->design_quality_score = Tools::getValue('attribute_design_quality_score');
        $object->wont_be_sold_again_this_year = Tools::isSubmit('wont_be_sold_again_this_year');
        $object->will_be_sold_again_next_year = Tools::isSubmit('will_be_sold_again_next_year');
        $object->wont_be_sold_again = Tools::isSubmit('wont_be_sold_again');
	}
	...
	public function postProcess($token = NULL)
	{
		...
		elseif (Tools::isSubmit('submitAttachments'))
		{
			if ($this->tabAccess['edit'] === '1')
                ...
                elseif (Tools::isSubmit('submitAddProductTag'))
		{
			if ($this->tabAccess['edit'] === '1')
                        {
                          //print_r($_POST['tags']);exit;
				if ($id = (int)(Tools::getValue($this->identifier)))
					if (Tag::addProductTags($id, $_POST['tags']))
						Tools::redirectAdmin($currentIndex.'&id_product='.(int)$id.(isset($_POST['id_category']) ? '&id_category='.(int)$_POST['id_category'] : '').'&conf=4&add'.$this->table.'&tabs=7&token='.($token ? $token : $this->token));
                        }
		}
		...
		/* Change search object showing status (active, inactive) */
		elseif (isset($_GET['showstatus']) AND Tools::getValue($this->identifier))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()))
				{
					if ($object->showingtogglestatus())
						Tools::redirectAdmin($currentIndex.'&conf=5'.((($id_category = (!empty($_REQUEST['id_category'])?$_REQUEST['id_category']:'1')) AND Tools::getValue('id_product')) ? '&id_category='.$id_category : '').'&token='.$token);
					else
						$this->_errors[] = Tools::displayError('An error occurred while updating status.');
				}
		...
		elseif (Tools::isSubmit('submitProductAttribute'))
		{
			if (Validate::isLoadedObject($product = new Product((int)(Tools::getValue('id_product')))))
			{
				...
				if (empty($_POST['attribute_reference']))
					$this->_errors[] = Tools::displayError('You must enter the Reference');
					...
					else
					{
						if ($this->tabAccess['add'] === '1')
						{
							if ($product->productAttributeExists($_POST['attribute_combinaison_list']))
								...
								Tools::getValue('attribute_supplier_reference'), Tools::getValue('attribute_ean13'), Tools::getValue('attribute_default'), Tools::getValue('id_cover_image'), Tools::getValue('attribute_location'), Tools::getValue('attribute_upc'));
						} else {
		...
		elseif (Tools::isSubmit('deleteProductAttribute'))
		{
			if ($this->tabAccess['delete'] === '1')
			{
				if (($id_product = (int)(Tools::getValue('id_product'))) AND Validate::isUnsignedId($id_product) AND Validate::isLoadedObject($product = new Product($id_product)))
				{
					...
					$product->checkDefaultAttributes();
					$product->updateQuantityProductWithAttributeQuantity($color);
					if (!$product->hasAttributes())
					{
		...
		elseif (Tools::isSubmit('updateProductCombinationStatus'))
		{
			if (Validate::isLoadedObject($product = new Product((int)(Tools::getValue('id_product')))))
			{
				$status = (int)Tools::getValue('updateProductCombinationStatus');
				$product->updateCombinationStatus((int)(Tools::getValue('id_product_attribute')), $status);
				Tools::redirectAdmin($currentIndex.'&add'.$this->table.'&id_category='.(!empty($_REQUEST['id_category'])?$_REQUEST['id_category']:'1').'&tabs=3&id_product='.$product->id.'&token='.($token ? $token : $this->token));
			}
			else
				$this->_errors[] = Tools::displayError('unable to the setting');
		}
		...
		elseif (Tools::isSubmit('submitPriceAddition'))
		{
			if ($this->tabAccess['add'] === '1')
			{
				...
				$strike_out = Tools::isSubmit('sp_strike_out')?1:0;
		...
		elseif (isset($_GET['position']))
		{
			...
			if (!$object->updatePosition((int)(Tools::getValue('way')), (int)(Tools::getValue('position'))))
				...
				Tools::redirectAdmin($currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&conf=5'.(($id_category = (!empty($_REQUEST['id_category'])?$_REQUEST['id_category']:'1')) ? ('&id_category='.$id_category) : '').'&token='.Tools::getAdminTokenLite('AdminCatalog'));
                   ...
                   elseif (isset($_GET['status'.$this->table]) AND Tools::getValue($this->identifier))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()))
		...
		/* change object showing status (active, inactive) */
		elseif (isset($_GET['showstatus'.$this->table]) AND Tools::getValue($this->identifier))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()))
		...
		try
		{
			...
			foreach (scandir($subdir) as $file)
			{
				...
				foreach ($imagesTypes AS $k => $imageType)
					...
					if (!Tools::resizeImage(_PS_PROD_IMG_DIR_.$image->getImgPath().'.jpg', _PS_PROD_IMG_DIR_.$image->getImgPath().'-'.stripslashes($imageType['name']).'.jpg', $imageType['width'], $imageType['height']))
					{
						$image->delete();
		...
		else
		{
			...
			elseif ($method == 'auto')
			{
				...
				foreach ($imagesTypes AS $k => $imageType)
					...
					if (!Tools::resizeImage($new_path.'.jpg', $new_path.'-'.stripslashes($imageType['name']).'.'.$image->image_format, $imageType['width'], $imageType['height']))
	...
	public function submitAddProduct($token = NULL)
	{
		...
		//if (Tools::getValue('collection_name_'.$language['id_lang']))

		/*To strip tags for collection name*/
		if (isset($_POST['collection_name']))
				$_POST['collection_name'] = strip_tags($_POST['collection_name']);
		...
		if (!sizeof($this->_errors))
		{
			...
			if (isset($id) AND !empty($id))
			{
				...
				if (Validate::isLoadedObject($object))
				{
					...
					// checking if there is a change in state
					$isActive = Tools::getValue('active', false);
					...
					$showing_status = Tools::getValue('showing_active', false);
					$object->showing_status = $showing_status;
	...
	public function displayForm($isMainTab = true)
	{
		...
		if ($obj->id)
			...
			<div class="tab-page" id="step7"><h4 class="tab">7. '.$this->l('Stylists').' ('.$countAttachments.')</h4></div>';
                        $product_tags  =  Module::hookExec('producttags', array('id_product' => $obj->id, 'idTab' => 8, 'content' => false));
                        echo $product_tags;
                        $toLoad = '';
                        if($product_tags)
                        {
                            $toLoad = "toload[8] = true;";
                        }
			/*Product Recommandation part*/
			$productRecommandation  =  Module::hookExec('productRecommend', array('id_product' => $obj->id, 'idTab' => 9, 'content' => false));
			echo $productRecommandation;
			$toLoadProductRecommand = '';
			if($productRecommandation){
				$toLoadProductRecommand = 'toload[9] = true;';
			}
			...
			echo ' 	if (toload[id]) {
							...
							$.post(
								"'.dirname($currentIndex).'/ajax.php", {
									ajaxProductTab: id, id_product: '.$obj->id.',loadTags : loadTag,
	...
	protected function _displaySpecificPriceAdditionForm($defaultCurrency, $shops, $currencies, $countries, $groups)
	{
		...
		<div id="add_specific_price" style="display: none;">
			...
			<label>'.$this->l('Show Old Price as striked out').'</label>
			<div class="margin-form">
				<input type="checkbox" name="sp_strike_out" value="0"/>
			</div>
			<div class="margin-form">
	...
	function displayFormAttachments($obj, $languages, $defaultLanguage)
	{
		...
		$attach2 = Attachment::getAttachments($cookie->id_lang, $obj->id, false);
				...
				echo '
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'<a href="index.php?tab=AdminStylists&addattachment&token='.Tools::getAdminToken('AdminStylists'.intval(Tab::getIdFromClassName('AdminStylists')).intval($cookie->id_employee)).'">
...
echo '<table>
	...
	function displayFormInformations($obj, $currency)
	{
		...
		echo '		</td>
					...
					<tr>
						<td class="col-left">'.$this->l('Collection Name:').'</td>
						<td style="padding-bottom:5px;" class="translatable">';
		foreach ($this->_languages as $language)
			echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
								<input size="50" type="text" id="collection_name_'.$language['id_lang'].'" name="collection_name_'.$language['id_lang'].'"
								value="'.stripslashes(htmlspecialchars($this->getFieldValue($obj, 'collection_name', $language['id_lang']))).'"'.((!$obj->id) ? ' onkeyup="if (isArrowKey(event)) return; copy2friendlyURL();"' : '').' onkeyup="if (isArrowKey(event)) return; updateCurrentText();" onchange="updateCurrentText();" /><sup> *</sup>
								<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
							</div>';
			...
			echo '	</td>
				...
				<table cellpadding="5" style="width: 45%; float: left; margin-left: 10px;">
					<tr>
						...
						<td style="padding-bottom:5px;">
							<input style="float:left;" onclick="toggleDraftWarning(false);showOptions(true);checkshowstatus(false)" type="radio" name="active" id="active_on" value="1" '.($this->getFieldValue($obj, 'active') ? 'checked="checked" ' : '').'/>
							...
							<br class="clear" />
							<input style="float:left;" onclick="toggleDraftWarning(true);showOptions(false);checkshowstatus(true)"  type="radio" name="active" id="active_off" value="0" '.(!$this->getFieldValue($obj, 'active') ? 'checked="checked" ' : '').'/>
					...
					<tr>
						<td style="vertical-align:top;text-align:right;padding-right:10px;font-weight:bold;">'.$this->l('Showing Status:').'</td>
						<td style="padding-bottom:5px;">
							<input style="float:left;" type="radio" name="showing_active" id="showing_active_on" value="1" '.($this->getFieldValue($obj, 'showing_status') ? 'checked="checked" ' : '').'/>
							<label for="active_on" class="t"><img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" style="float:left; padding:0px 5px 0px 5px;" />'.$this->l('Enabled').'</label>
							<br class="clear" />
							<input style="float:left;" type="radio" name="showing_active" id="showing_active_off" value="0" '.(!$this->getFieldValue($obj, 'showing_status') ? 'checked="checked" ' : '').'/>
		...
		echo '
					...
					<tr>
						<td style="vertical-align:top;text-align:right;padding-right:10px;font-weight:bold;">'.$this->l('Product Type:').'</td>
						<td style="padding-bottom:5px;">
							<select name="id_product_type" id="id_product_type">
							<option value="0">'.$this->l('unknown').'</option>';
		  $product_types = Product::getProductTypes();
		  if($product_types)
			foreach($product_types as $type)
				if($type['id_product_type'] !=0 )
			echo '<option value="'.$type['id_product_type'].'" '.($this->getFieldValue($obj, 'id_product_type') == $type['id_product_type'] ? 'selected="selected"' : '').'>'.$type['type_name'].'</option>';
		echo '</select>&nbsp;&nbsp;&nbsp;
						</td>
					</tr>
					<tr>
						<td style="vertical-align:top;text-align:right;padding-right:10px;font-weight:bold;">' . $this->l("Won't Be Sold This Year") . '</td>
						<td>
							<input type="checkbox" name="wont_be_sold_again_this_year" value="1" ' . ($this->getFieldValue($obj, 'wont_be_sold_again_this_year') ? 'checked="checked" ' : '') . ' />
						</td>
					</tr>
					<tr>
						<td style="vertical-align:top;text-align:right;padding-right:10px;font-weight:bold;">' . $this->l("Will Be Sold Again Next Year") . '</td>
						<td>
							<input type="checkbox" name="will_be_sold_again_next_year" value="1" ' . ($this->getFieldValue($obj, 'will_be_sold_again_next_year') ? 'checked="checked" ' : '') . ' />
						</td>
					</tr>
					<tr>
						<td style="vertical-align:top;text-align:right;padding-right:10px;font-weight:bold;">' . $this->l("Won't Be Sold Again") . '</td>
						<td>
							<input type="checkbox" name="wont_be_sold_again" value="1" ' . ($this->getFieldValue($obj, 'wont_be_sold_again') ? 'checked="checked" ' : '') . ' />
						</td>
					</tr>';
			echo '
				</table>
...
<?php
				...
				echo '
					...
					echo '<tr>
			  <td class="col-left">'.$this->l('Design Quality Score:').'</td>';

			  echo '<td style="padding-bottom:5px;"><select name="attribute_design_quality_score">';
			  echo '<option value="0">--</option>';

			  for($i = 1; $i < 11; $i++) {
			  	if($this->getFieldValue($obj, 'design_quality_score') != $i) {
			  ...
			  echo '</select></td>
					...
					#var_dump($this->getFieldValue($obj, 'id_product')); exit();
	...
	public function initCombinationImagesJS()
	{
		...
		foreach ($allCombinationImages AS $id_product_attribute => $combinationImages)
		{
			...
			foreach ($combinationImages AS $combinationImage){
					((isset($combinationImage['id_image']))?$content .= 'combination_images['.(int)($id_product_attribute).']['.$i++.'] = '.(int)($combinationImage['id_image']).';':'');
			}
		}
		return $content;
	}
	...
	function displayFormAttributes($obj, $languages, $defaultLanguage)
	{
		...
		$default_country = new Country((int)Configuration::get('PS_COUNTRY_DEFAULT'));
		$images = Image::getImages((int)($cookie->id_lang), $obj->id);
//		echo "<pre>";print_r($attributes_groups);exit;
//		echo "<pre>";print_r($attributeJs);exit;
		...
		echo'
		  ...
		  <tr>
			...
			foreach ($images AS $image)
			{
				...
				<label for="id_image_attr_'.(int)($image['id_image']).'" style="float: none;"><img src="'._THEME_PROD_DIR_.$imageObj->getExistingImgPath().'-small.jpg" alt="'.htmlentities(stripslashes($image['legend']), ENT_COMPAT, 'UTF-8').'" title="'.htmlentities(stripslashes($image['legend']), ENT_COMPAT, 'UTF-8').'" /></label>
				<span class="cover" style="float: left; padding: 5px 0 0 25px;"><input type="checkbox" name="id_cover_image[]" value="'.(int)($image['id_image']).'" id="id_cover_image_'.(int)($image['id_image']).'" />&nbsp;cover</span></li>';
				++$i;
			}
			echo '</ul>
			...
			if ($obj->id)
			{
				...
				if (is_array($combinaisons))
				{
					...
					foreach ($combinaisons AS $k => $combinaison)
					{
						...
						if(isset($combinationImages[$combinaison['id_product_attribute']]['coverimage']))
						foreach($combinationImages[$combinaison['id_product_attribute']]['coverimage'] as $combination)
						{
							if($combination>0)
							{
								$combArray[$combinaison['id_product_attribute']]['cover_image'] = (int)($combination);
								if ($combinaison['is_color_group'])
									$setcover[$combinaison['id_attribute']] = $combinaison['id_product_attribute'];
							}
						}
						$combArray[$combinaison['id_product_attribute']]['default_on'] = $combinaison['default_on'];
						$combArray[$combinaison['id_product_attribute']]['active'] = $combinaison['active'];
						...
						if ($combinaison['is_color_group']){
							...
							$attribute_ipa[$combinaison['id_attribute']][]= $combinaison['id_product_attribute'];
							$color_product = 2;
						}
					}
				}
//				print_r($attribute_ipa);echo "<br/>";exit;
//				print_r($setcover);echo "<br/>";exit;
//				echo "<pre>";print_r($combArray);echo "<br/>";exit;
				$irow = 0;
				if (isset($combArray))
				{
					foreach ($combArray AS $id_product_attribute => $product_attribute)
					{
						...
						if(isset($attribute_ipa)){
							if(!isset($setcover[$color_id]) && !isset($coverSet[$color_id]))
							{
								if($product_attribute['active'])
								{
									if(in_array($id_product_attribute, $attribute_ipa[$color_id]))
									{
										$cover_exists = TRUE;
										$coverSet[$color_id] = 1;
									}
						...
						if(isset($setcover[$color_id]) && $setcover[$color_id] == $id_product_attribute && $product_attribute['active'])
						{
							$cover_exists = TRUE;
							$coverSet[$color_id] = 1;
						}
						...
						if(isset($product_attribute['cover_image']))
						{
							$cover = $product_attribute['cover_image'];
							//$test = TRUE;
						}
						//$cover=(isset($product_attribute['cover_image'])?$product_attribute['cover_image']:0);
						echo '
						<tr'.($irow++ % 2 ? ' class="alt_row"' : '').($product_attribute['default_on'] ? ' style="background-color:#D1EAEF"' : '').'>
							...
							\''.$product_attribute['quantity'].'\', \''.($attrImage ? $attrImage->id : 0).'\', Array('.$jsList.'), \''.$id_product_attribute.'\', \''.$product_attribute['default_on'].'\', \''.$product_attribute['ecotax'].'\', \''.$product_attribute['location'].'\', \''.$product_attribute['upc'].'\', \''.$product_attribute['minimal_quantity'].'\', \''.$cover.'\', \''.$cover_exists.'\'); calcImpactPriceTI();" /></a>&nbsp;
							...
							<td class="center">
							<a href="'.$currentIndex.'&updateProductCombinationStatus='.($product_attribute['active'] ? 0 : 1).'&id_product_attribute='.$id_product_attribute.'&id_product='.$obj->id.'&'.(Tools::isSubmit('id_category') ? 'id_category='.(int)(Tools::getValue('id_category')).'&' : '&').'token='.Tools::getAdminToken('AdminCatalog'.(int)(Tab::getIdFromClassName('AdminCatalog')).(int)($cookie->id_employee)).'">
								<img src="../img/admin/'.($product_attribute['active'] ? 'enabled.gif' : 'disabled.gif').'"
								alt="'.($product_attribute['active'] ? $this->l('Enabled') : $this->l('Disabled')).'" title="'.($product_attribute['active'] ? $this->l('Enabled') : $this->l('Disabled')).'" />
							</a>
							</td>
						</tr>';
					}
