...
class AdminCategories extends AdminTab
{
	...
	public function displayForm($token = NULL)
	{
		...
		foreach ($this->_languages AS $language)
		echo '<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
					<input type="text" name="header_'.$language['id_lang'].'" id="header_'.$language['id_lang'].'" value="'.htmlentities($this->getFieldValue($obj, 'header', (int)($language['id_lang'])), ENT_COMPAT, 'UTF-8').'" />
			  </div>';
		echo '	<p class="clear"></p>
				</div>
				<label>'.$this->l('Header Text:').' </label>
				<div class="margin-form translatable">';
		foreach ($this->_languages AS $language)
		echo '<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
					<input type="text" name="header_text_'.$language['id_lang'].'" id="header_text_'.$language['id_lang'].'" value="'.htmlentities($this->getFieldValue($obj, 'header_text', (int)($language['id_lang'])), ENT_COMPAT, 'UTF-8').'" />
			  </div>';
		echo '	<p class="clear"></p>
				</div>
