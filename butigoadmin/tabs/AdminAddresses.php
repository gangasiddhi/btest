			...
			';
				...
				echo '
					...
					$(document).ready(function(){
							ajaxStates(ajaxProvinces);

							$(\'#id_country\').change(function() {
								ajaxStates ();
								});
							function ajaxStates (callback)
							{
								$.ajax({
									url: "ajax.php",
									cache: false,
					...
					function ajaxProvinces() {
					    $.ajax({
					        url: "ajax.php",
					        cache: false,
					        data: "ajaxProvinces=1&id_country="+$(\'#id_country\').val()+"&id_state="+$(\'#id_state\').val(),
					        success: function(html) {
					            if (html == \'false\')
					            {
					                $("#contains_provinces").fadeOut();
					                $(\'#id_province option[value=0]\').attr("selected", "selected");
					            } else {
					                $("#id_province").html(html);
					                $("#contains_provinces").fadeIn();
					                $(\'#id_province option[value='.(int)$obj->id_province.']\').attr("selected", "selected");
					            }
					        }
					    });
					}
