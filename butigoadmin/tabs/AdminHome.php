	  ...
	  echo '</div>';
		...
		<div id="column_left">
			...
			$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				SELECT SUM(o.`total_paid_real` / o.`conversion_rate`) as total_sales,
					COUNT(*) as total_orders
				FROM `' . _DB_PREFIX_ . 'orders` o
				WHERE o.`valid` = 1
				AND o.`date_add` BETWEEN \''.date('Y-m').'-01 00:00:00\' AND \''.date('Y-m').'-31 23:59:59\' ');
			...
			<div class="table_info">
				<h5>'.$this->l('Monthly Statistics').' </h5>
			...
			echo '<div id="table_info_large">
				<h5>'.'<strong>'.$this->l('Statistics').'</strong> / '.$this->l('Sales of the week').'</h5>
				<div id="stat_google">';

		define('PS_BASE_URI', __PS_BASE_URI__);
		$chart = new Chart();
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
			SELECT o.`total_paid` / o.`conversion_rate` as total_converted,
				o.`date_add`
			FROM `'._DB_PREFIX_.'orders` o
			WHERE o.`valid` = 1
			AND o.`date_add` BETWEEN \''.date('Y-m-d', strtotime('-7 DAYS', time())).' 00:00:00\' AND \''.date('Y-m-d H:i:s').'\'');

		foreach ($result as $row) {
			$chart->getCurve(1)->setPoint(strtotime($row['date_add']), $row['total_converted']);
		}
