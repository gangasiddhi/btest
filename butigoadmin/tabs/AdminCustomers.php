...
class AdminCustomers extends AdminTab
{
	public function __construct()
	{
		...
		$this->_select = '(
			...
			LEFT JOIN '._DB_PREFIX_.'orders od ON od.id_order = ord.id_order
		...
		'ord_qty' =>  array('title' => $this->l('Ord.'), 'align' => 'center', 'width' => 25,'search' => false),
		'items_qty' =>  array('title' => $this->l('Qty.'), 'align' => 'center', 'width' => 25,'search' => false),
		'rev' =>  array('title' => $this->l('Rev.'), 'align' => 'center','width' => 25,'search' => false),
	...
	public function viewcustomer()
	{
		...
		$paginationParams = array(
			order => array(
				itemPerPage => Configuration::get("DEFAULT_ORDER_LIMIT_PAGING"),
				totalItem => 0,
				pageNo => Tools::getValue("orderPagination") ? Tools::getValue("orderPagination") : 1
			),
			discount => array(
				itemPerPage => Configuration::get("DEFAULT_DISCOUNT_LIMIT_PAGING"),
				totalItem => 0, // fill below
				pageNo => Tools::getValue("discountPagination") ? Tools::getValue("discountPagination") : 1
			),
			product => array(
				itemPerPage => Configuration::get("DEFAULT_PRODUCT_LIMIT_PAGING"),
				totalItem => 0, // fill below
				pageNo => Tools::getValue("productPagination") ? Tools::getValue("productPagination") : 1
			),
			cart => array(
				itemPerPage => Configuration::get("DEFAULT_CART_LIMIT_PAGING"),
				totalItem => 0, // fill below
				pageNo => Tools::getValue("cartPagination") ? Tools::getValue("cartPagination") : 1
			),
			pendingFPagination => array(
		...
		$products = $customer->getBoughtProducts(true,  $paginationParams[product][itemPerPage], $paginationParams[product][pageNo] );
		$paginationParams[product][totalItem] = $products['totalItem'];
		unset($products['totalItem']);

		$discounts = Discount::getCustomerDiscounts($defaultLanguage, (int)$customer->id, false, false, false,
			true, $paginationParams[discount][itemPerPage], $paginationParams[discount][pageNo] );
		$paginationParams[discount][totalItem] = $discounts['totalItem'];
		unset($discounts['totalItem']);

		$orders = Order::getCustomerOrders((int)$customer->id, true,
			true,  $paginationParams[order][itemPerPage], $paginationParams[order][pageNo] );
		$paginationParams[order][totalItem] = $orders['totalItem'];
		unset($orders['totalItem']);

		$carts = Cart::getCustomerCarts((int)$customer->id, true, $paginationParams[cart][itemPerPage], $paginationParams[cart][pageNo]);
		$paginationParams[cart][totalItem] = $carts['totalItem'];
		unset($carts['totalItem']);
		...
		// pagination parameter pendingFPagination
		$hook = Module::hookExec('adminCustomers', array('id_customer' => $customer->id,
			"pagination" => array( "itemPerPage" => $paginationParams[pendingFPagination][itemPerPage],
					"pageNo" => $paginationParams[pendingFPagination][pageNo])));
		...
		if ($orders AND sizeof($orders))
		{
			...
			<table cellspacing="0" cellpadding="0" class="table float">
				...
				if ($paginationParams[order][totalItem] > $paginationParams[order][itemPerPage]) {
					echo '<div id="order-pagination" class="pagination butigo-pagination green-pagination"></div>';
		...
		if ($products AND sizeof($products))
		{
			...
			if ($paginationParams[product][totalItem] > $paginationParams[product][itemPerPage]) {
				echo '<div id="product-pagination" class="pagination butigo-pagination green-pagination"></div>';
				echo '<script type="text/javascript">
					$(function(){
						$("#product-pagination").pagination('.$paginationParams[product][totalItem].', {
		                    prev_text: "'.$this->l("Prev").'"
		                    , next_text: "'.$this->l("Next").'"
		                    , items_per_page: '.$paginationParams[product][itemPerPage].'
		                    , num_display_entries: 10
		                    , num_edge_entries: 2
		                    , current_page : '.($paginationParams[product][pageNo] - 1).'
		                    , callback: function(pageNo, $pagination) {
		                        if (pageNo == '.$paginationParams[product][pageNo].' - 1) return;
								pageNo = parseInt(pageNo) + 1;
								var urlParams =  getQueryString();
								urlParams["productPagination"] = pageNo;
								goToUrl(location.pathname + "?" + $.param(urlParams));
								return false;
		                    }
		                });
		...
		if (sizeof($addresses))
		{
			...
			foreach ($addresses AS $address)
				...
				<tr '.($irow++ % 2 ? 'class="alt_row"' : '').'>
					...
					<td>'.$address['address1'].($address['address2'] ? ' '.$address['address2'] : '').' '.$address['postcode'].' '.$address['cityName'].'</td>
		...
		if (sizeof($discounts))
		{
			...
			if ($paginationParams[discount][totalItem] > $paginationParams[discount][itemPerPage]) {
				echo '<div id="discount-pagination" class="pagination butigo-pagination green-pagination"></div>';
				echo '<script type="text/javascript">
					$(function(){
						$("#discount-pagination").pagination('.$paginationParams[discount][totalItem].', {
		                    prev_text: "'.$this->l("Prev").'"
		                    , next_text: "'.$this->l("Next").'"
		                    , items_per_page: '.$paginationParams[discount][itemPerPage].'
		                    , num_display_entries: 10
		                    , num_edge_entries: 2
		                    , current_page : '.($paginationParams[discount][pageNo] - 1).'
		                    , callback: function(pageNo, $pagination) {
		                        if (pageNo == '.$paginationParams[discount][pageNo].' - 1) return;
								pageNo = parseInt(pageNo) + 1;
								var urlParams =  getQueryString();
								urlParams["discountPagination"] = pageNo;
								goToUrl(location.pathname + "?" + $.param(urlParams));
								return false;
		                    }
		                });
		...
		if ($carts AND sizeof($carts))
		{
			...
			if ($paginationParams[cart][totalItem] > $paginationParams[cart][itemPerPage]) {
				echo '<div id="cart-pagination" class="pagination butigo-pagination green-pagination"></div>';
				echo '<script type="text/javascript">
					$(function(){
						$("#cart-pagination").pagination('.$paginationParams[cart][totalItem].', {
		                    prev_text: "'.$this->l("Prev").'"
		                    , next_text: "'.$this->l("Next").'"
		                    , items_per_page: '.$paginationParams[cart][itemPerPage].'
		                    , num_display_entries: 10
		                    , num_edge_entries: 2
		                    , current_page : '.($paginationParams[cart][pageNo] - 1).'
		                    , callback: function(pageNo, $pagination) {
		                        if (pageNo == '.$paginationParams[cart][pageNo].' - 1) return;
								var urlParams =  getQueryString();
								urlParams["cartPagination"] = parseInt(pageNo) + 1;
								goToUrl(location.pathname + "?" + $.param(urlParams));
								return false;
		                    }
		                });
