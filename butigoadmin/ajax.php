...
if ($step = (int)(Tools::getValue('ajaxProductTab')))
{
	...
	$switchArray = array(3 => 'displayFormPrices', 4 => 'displayFormAttributes', 5 => 'displayFormFeatures', 6 => 'displayFormCustomization', 7 => 'displayFormAttachments');
        if(Tools::getIsset('loadTags') && Tools::getValue('loadTags') == 1)
        {
            echo Module::hookExec('producttags', array('product' => $product, 'languages' => $languages, 'defaultLanguage' => $defaultLanguage , 'content' => true));
        }else if(Tools::getIsset('loadTags') && Tools::getValue('loadTags') == 2)
        {
            echo Module::hookExec('productRecommend', array('product' => $product, 'languages' => $languages, 'defaultLanguage' => $defaultLanguage , 'content' => true));
        }
	$currentIndex = 'index.php?tab=AdminCatalog';
...
if (array_key_exists('ajaxProductsPositions', $_POST))
{
	...
	Module::hookExec('updateProductPosition', array(
...
if (isset($_GET['ajaxProvinces']) AND isset($_GET['id_state']))
{
	$id_province = Tools::getValue('id_state');
	$sql = 'SELECT p.id_province, p.name FROM '._DB_PREFIX_.'province p LEFT JOIN '._DB_PREFIX_.'state s ON (p.`id_state` = s.`id_state`)
	WHERE p.id_state = '.$id_province.' AND s.active = 1 ORDER BY p.`name` ASC';
	$provinces = Db::getInstance()->ExecuteS($sql);

	if (is_array($provinces) AND !empty($provinces))
	{
		$list = '';
		if (Tools::getValue('no_empty') != true)
			$list = '<option value="0">-----------</option>'."\n";
		...
		foreach ($provinces AS $item)
			$list .= '<option value="'.(int)($item['id_province']).'"'.((isset($id_province) AND $id_province == $item['id_state']) ? ' selected="selected"' : '').'>'.$item['name'].'</option>'."\n";
	}
	else
		$list = 'false';
...
if (Tools::getValue('id_order') AND Tools::getValue('tracking_number')) {
	$iOrder =  new Order(Tools::getValue('id_order'));
	$iOrder->tracking_number = Tools::getValue('tracking_number');

	if ($iOrder->save()) {
		die(json_encode(array("success" => true)));
	} else {
		die(json_encode(array("error" => array("message" => "Error during update tracking number"))));
	}
}
