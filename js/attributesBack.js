...
function fillCombinaison(wholesale_price, price_impact, weight_impact, unit_impact, reference, supplier_reference, ean, quantity, image, old_attr, id_product_attribute, default_attribute, eco_tax, location, upc, minimal_quantity, cover_image, cover_exists)
{
	init_elems();
	...
	coverImages = $('#id_image_attr').find("input[id^=id_cover_image_]");
	coverImages.each(function() {
		this.checked = false;
	});
	if(cover_image != 0 && cover_exists== true)
	{
		/* Check cover image */
		$('span.cover').show();
		if(cover_image)
			getE('id_cover_image_' + cover_image).checked = true;

	}
	else if(cover_exists== true)
	{
		$('span.cover').show();
	}
	else if(cover_exists== false)
	{
		$('span.cover').hide();
	}
