...
class AddressFormatCore extends ObjectModel
{
	...
	*/
	public static function getFormattedLayoutData($address)
	{
		$layoutData = array();
		/*to fix IE7 issue*/
		$notrequiredaddressfields = array('company','vat_number', 'address2', 'postcode city');
		/*to fix IE7 issue*/
		if ($address && $address instanceof Address)
		{
			$layoutData['ordered'] = AddressFormat::getOrderedAddressFields((int)$address->id_country);
			/*
			$layoutData['ordered'][6] = 'province';
			$layoutData['ordered'][7] = 'state';
			$layoutData['ordered'][8] = 'Country:name';
			$layoutData['ordered'][9] =	'phone';
			*/

			/*to fix IE7 issue*/
			//$layoutData['ordered'] = array_diff($layoutData['ordered'], $notrequiredaddressfields);
			/*to fix IE7 issue*/
			$layoutData['formated'] = AddressFormat::getFormattedAddressFieldsValues($address, $layoutData['ordered']);
			$layoutData['formated']['state'] = State::getNameById((int)$address->id_state);
			$layoutData['formated']['province'] = Province::getProvinceNameById((int)$address->id_province);
