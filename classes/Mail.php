...
class MailCore {
    public static function Send($id_lang, $template, $subject, $templateVars, $to, $toName = NULL, $from = NULL,
        $fromName = NULL, $fileAttachment = NULL, $modeSMTP = NULL, $templatePath = _PS_MAIL_DIR_, $die = false,
        $logo = true) {
        ...
        if (! isset($configuration['PS_MAIL_SMTP_ENCRYPTION'])) {
            ...
            $log->debug('SMTP Encryption off!');
        }

        if (! isset($configuration['PS_MAIL_SMTP_PORT'])) {
            $configuration['PS_MAIL_SMTP_PORT'] = 'default';
            $log->debug('SMTP port is "default"!');
        }

        if (! isset($from)) {
            $from = $configuration['PS_SHOP_EMAIL'];
            $log->debug('From e-mail: ' . $from);
        }
        ...
        if (! empty($from) AND ! Validate::isEmail($from)) {
            $msg = 'Error: parameter "from" is corrupted';
        ...
        if (! empty($fromName) AND ! Validate::isMailName($fromName)) {
            $msg = 'Error: parameter "fromName" is corrupted';
        ...
        if (! is_array($to) AND ! Validate::isEmail($to)) {
            $msg = 'Error: parameter "to" is corrupted';
        ...
        if (! is_array($templateVars)) {
            $msg = 'Error: parameter "templateVars" is not an array';
        ...
        if (! empty($toName) AND ! Validate::isMailName($toName)) {
            $toName = NULL;
            $log->debug('Set toName to NULL!');
        }

        if (! Validate::isTplName($template)) {
            $msg = 'Error: invalid email template';
        ...
        if (! Validate::isMailSubject($subject)) {
            $msg = 'Error: invalid email subject';
        ...
        if (is_array($to)) {
            $log->debug('$to is an array: ' . print_r($to, true));
            ...
            $log->debug('$to is not an array: ' . $to);
        ...
        try {
            ...
            if ($configuration['PS_MAIL_METHOD'] == 2) {
                ...
                if (! $connection) {
                    $log->debug('Cannot establish a connection to mail server!');
            ...
            if (! $iso) {
                $msg = 'Error - No ISO code for email';
            ...
            if ($moduleName !== false
                AND (
                    file_exists(_PS_THEME_DIR_ . 'modules/' . $moduleName . '/mails/' . $template . '.txt')
                    OR file_exists(_PS_THEME_DIR_ . 'modules/' . $moduleName . '/mails/' . $template . '.html')
                )) {
                ...
                $msg = 'Error - The following email template is missing:' . ' ' . $templatePath . $template . '.txt';
            ...
            if ($logo == true) {
                $templateVars['{shop_logo}'] = file_exists(_PS_IMG_DIR_ . 'logo_mail.jpg') ?
                    $message->attach(new Swift_Message_Image(new Swift_File(_PS_IMG_DIR_ . 'logo_mail.jpg'))) :
                    (
                        file_exists(_PS_IMG_DIR_ . 'logo.jpg') ?
            ...
            $log->debug('Mail is successfully sent!');
