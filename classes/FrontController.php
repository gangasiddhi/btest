...
class FrontControllerCore
{
    ...
    protected static $hypothesis = NULL;
    public $hypothesis_design = NULL;
    public $auth = false;
    ...
    {
        global $cookie, $utm_params, $smarty, $cart, $iso, $defaultCountry, $protocol_link, $protocol_content, $link, $css_files, $js_files, $hypothesis_design, $new_checkout_process;
        ...
        /*Checking the whether the site open for all or not, Start*/
		$showSiteSetVariables = array('from_gad' => Tools::getValue('from_gad'));
		$cookie->show_site = Tools::getSiteShownvaribale($showSiteSetVariables);
		if($cookie->show_site == 1){
			$smarty->assign('show_site', 1);
		}else{
			$smarty->assign('show_site', 0);
		}

        /*Checking the whether the site open for all or not, End*/

        $utm_params = '';
        if (Tools::getIsset('utm_campaign') AND Tools::getValue('utm_campaign'))
            $utm_params .= 'utm_campaign='.Tools::getValue('utm_campaign');
        if (Tools::getIsset('utm_medium') AND Tools::getValue('utm_medium'))
            $utm_params .= '&utm_medium='.Tools::getValue('utm_medium');
        if (Tools::getIsset('utm_source') AND Tools::getValue('utm_source'))
            $utm_params .= '&utm_source='.Tools::getValue('utm_source');
        if (Tools::getIsset('utm_content') AND Tools::getValue('utm_content'))
            $utm_params .= '&utm_content='.Tools::getValue('utm_content');
        if($utm_params == '')
        {
            if (Tools::getValue('utm_params'))
                $utm_params = Tools::getValue('utm_params');
//          echo $utm_params.'<br>';//exit;
        }
        $smarty->assign('utm_params', Tools::safeOutput($utm_params));
//echo 'FC'.$utm_params; //exit;

        /* get page name to display it in body id */
        $page_name = (isset($this->php_self) ? preg_replace('/\.php$/', '', $this->php_self) : '');

        /*hypothesis*/
        /*if($this->hypothesis_design  == 'showroom')
        {
            //$customer = new Customer($cookie->id_customer);
            //if($cookie->isLogged() AND strtotime($customer->date_add) >  strtotime(date('2011-12-7 00:00:00')))
                self::$hypothesis = Hypothesis::setUpHypothesis($this->hypothesis_design , 'cookie');
                if( _BU_ENV_ == 'production')
prod                {
                    $errorLogFile = @fopen(_PS_ROOT_DIR_.'/log/hypothesis_track.txt', "a");
                    fwrite($errorLogFile,'START');
                    fwrite($errorLogFile, sprintf("\n%s:- %s,%s,%s\n",date("D M j G:i:s T Y").'-New Customer.First Login','Design Name:-'.$this->hypothesis_design,'Hypothesis Key Value:-'.self::$hypothesis,'Cookie:-'.$cookie->hypo_keys ."\n"));
                    fclose($errorLogFile);
                }
        }
        elseif($related = Hypothesis::returnRelatedHypothesis($page_name, 'cookie'))
        {
                self::$hypothesis = $related;
                if( _BU_ENV_ == 'production')
                {
                    $errorLogFile = @fopen(_PS_ROOT_DIR_.'/log/hypothesis_track.txt', "a");
                    fwrite($errorLogFile, sprintf("\n%s:- %s,%s,%s\n",date("D M j G:i:s T Y").'-Registered Customer','Design Name:-'.'showroom','Hypothesis Key Value:-'.self::$hypothesis, 'Cookie:-'.$cookie->hypo_keys ."\n"));
                    fwrite($errorLogFile,'END'."\n");
                    fclose($errorLogFile);
                }
        }*/
        /*hypothesis*/
        ...
        if ($ref_by = Tools::getValue('ref_by')) {
        ...
        if (isset($_GET['logout']) OR ($cookie->logged AND Customer::isBanned((int)$cookie->id_customer))) {
            ...
            $cookie->mylogout();
            //Tools::redirect(isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : NULL);
			/*To redirect to logged-out.php (loland) on logged out from any of the page*/
			Tools::redirect('logged-out.php');
        }
        ...
        $smarty->assign(array(
            ...
            'isLiveChatEnabled' => Tools::getValue('specialCode') === Tools::encrypt('specialTemporaryLiveChat')
        ));
        ...
        // Assign app-Id
        if(_BU_ENV_ == 'production')
            $app_id = 220196491340962;
        elseif(_BU_ENV_ == 'development')
            $app_id = 312142095529326;
        else
            $app_id = 220196491340962;
        ...
        if (Tools::isSubmit('live_edit') AND $ad = Tools::getValue('ad') AND (Tools::getValue('liveToken') == sha1(Tools::getValue('ad')._COOKIE_KEY_)))
            if (!is_dir(_PS_ROOT_DIR_.DIRECTORY_SEPARATOR.$ad))
                die(Tools::displayError('Error: 201306051440'));

        /*BEGINS -- Checkout Process cohort*/
        if (self::$cookie->isLogged() && intval(self::$cookie->id_customer)) {
            ...
            foreach($groups AS $group) {
                if(strpos($group['name'], 'NewCheckout') !== false)
                    $group_id_new_checkout_process = $group['id_group'];

                if(strpos($group['name'], 'OldCheckout') !== false){
                    $group_id_old_checkout_process = $group['id_group'];
                    $smarty->assign('group_id_Old_checkout_cohort_process', $group_id_old_checkout_process);
                }
            }

            if($customer->isMemberOfGroup($group_id_new_checkout_process)) {
                $new_checkout_process = 1;
                //self::$smarty->assign('checkout_header', 1);
                if($page_name == 'order' && (Tools::getValue('step') >= 2)) {
                    $new_checkout_process = 2;
                    self::$smarty->assign('checkout_footer', 1);
                }
            }
        }
        /*ENDS -- Checkout Process chort*/

        /*BEGINS -- Personyze cohort*/
        if (self::$cookie->isLogged() && intval(self::$cookie->id_customer)) {
            $customer = new Customer(intval(self::$cookie->id_customer));
            $groups = Group::getGroups(self::$cookie->id_lang);
            foreach($groups AS $group) {
                if(strpos($group['name'], 'SelfPersonyze') !== false) {
                    $group_id_personyze = $group['id_group'];
                    if($customer->isMemberOfGroup($group_id_personyze))
                       self::$smarty->assign('personyze', 1);
                }
            }
        }
        /*ENDS -- Personyze chort*/
    ...
    {
        self::$smarty->assign( 'has_title', false );

        /*Showing an Accessory Banner*/
        if((int)self::$cookie->logged && isset($_COOKIE['jand'])) {
            if (strpos($_SERVER['PHP_SELF'], 'accessoriesed-products') === false AND strpos($_SERVER['PHP_SELF'], 'showroom') === false AND strpos($_SERVER['PHP_SELF'], 'waiting') === false) {
                self::$smarty->assign('show_jand', 1);
                setcookie('jand', "",time() - 3600);
            }
        }
    ...
    {
        global $cookie, $new_checkout_process;

        Tools::addCSS(_THEME_CSS_DIR_.'global.css', 'all');

        if ($new_checkout_process == 2) {
            Tools::addCSS(_THEME_CSS_DIR_.'global-checkout.css', 'all');
        }
    ...
    public function displayHeader()
    {
        ...
        //if (_BU_ENV_ == 'production')
            //require_once(_PS_TOOL_DIR_.'ClickTale/ClickTaleTop.php');

        // P3P Policies (http://www.w3.org/TR/2002/REC-P3P-20020416/#compact_policies)
        header('P3P: CP="IDC DSP COR CURa ADMa OUR IND PHY ONL COM STA"');

        // Cache-Control Header (for varnish -- duration: half an hour)
        ...
        self::$smarty->assign(array(
			...
			'HOOK_HEADER_EXTERNAL_SCRIPTS' => Module::hookExec('headerExternalScripts'),
    ...
    public function displayFooter()
    {
        ...
        self::$smarty->assign(array(
			...
			'HOOK_FOOTER_EXTERNAL_SCRIPTS' => Module::hookExec('footerExternalScripts'),
            'content_only' => (int)(Tools::getValue('content_only')),
            'copy_year' => '2011-'.$copy_year,
        ));
		self::$smarty->display(_PS_THEME_DIR_.'footer.tpl');

        //if (_BU_ENV_ == 'production')
        //  require_once(_PS_TOOL_DIR_.'ClickTale/ClickTaleBottom.php');
    ...
    /*To make the dicounted products disapper from all Showrooms, if the customer brought that product*/
    public function disappearDiscountedProducts($products)
    {

        //Checking whether the customer brought products are moved to discount
        $discount_product_ids = Product::discountedProductslist();
        $customer_brought_products = Customer::customerBroughtProductslist(self::$cookie->id_customer);
        $brought_products_in_discount = array_intersect($discount_product_ids, $customer_brought_products);
        //echo "<pre>"; print_r($brought_products_in_discount); echo "</pre>";

        // Deleting the customer brought discounted products from the supllied product list(like Showroom,Lookbook, Ivana, Discount Botique)
        if(!empty($brought_products_in_discount))
        {
            foreach($products as $key=>$product)
                if(in_array($product['id_product'], $brought_products_in_discount))
                        unset($products[$key]);
        }

        /*if(!empty($brought_products_in_discount))
            return $brought_products_in_discount;
        else
            return false;*/
