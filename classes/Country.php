...
class CountryCore extends ObjectModel
{
	...
	/**
	  ...
	  *
	  * @param integer $id_lang Language ID
	  * @return array Provinces and corresponding zones
	  */
	static public function getProvinces()
	{

		$provinces = Db::getInstance()->ExecuteS('
		SELECT p.*
		FROM `'._DB_PREFIX_.'province` p
		ORDER BY p.`name` ASC');
		$result = Db::getInstance()->ExecuteS('
			SELECT s.*
		FROM `'._DB_PREFIX_.'state` s');
		$states = array();
		foreach ($result AS &$statev)
			$states[$statev['id_state']] = $statev  ;
		foreach ($provinces AS &$province)
			if (isset($states[$province['id_state']]))
				$states[$province['id_state']]['provinces'][] = $province;
