...
class	CookieCore
{
    ...
    public function setVarnishCookie($enable=true) {
        if ($enable) {
            setcookie("varnishLogged", "1", $this->_expire, $this->_path, $this->_domain, 0, false);
        } else {
            setcookie("varnishLogged", "1", time() - 100, $this->_path, $this->_domain, 0, false);
        }
    }
    ...
    {
        $domain = BCache::get("butigoCookieDomain");
        ...
        if (!$domain) {
            ...
            BCache::set("butigoCookieDomain", $domain, 0);
        }
		return $domain;
	}
	...
	{
		if (is_array($value)) {
			error_log('Given parameter should not be an array!');
			return;
		}
   ...
   public function isLogged($withGuest = false)
   {
       ...
       if ($this->logged == 1 AND $this->id_customer
           AND Validate::isUnsignedId($this->id_customer)
           AND Customer::checkPassword((int)($this->id_customer), $this->passwd)) {                                                 
	...
	public function mylogout()
	{
		...
		unset($this->_content['refer_type']);
		unset($this->_content['ref_by']);
		unset($this->_content['takeSurvey']);
		unset($this->_content['id_favlist']);
		unset($this->_content['from_gad']);
		unset($this->_content['show_site']);
		//unset($this->_content['referrer']);
		//if(isset($this->_content['hypo_keys']))
		//unset($this->_content['hypo_keys']);
		$this->_modified = true;
		$this->write();
	}
