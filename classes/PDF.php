...
class PDFCore extends PDF_PageGroupCore
{
	...
	protected static $_fpdf_core_fonts = array('courier', 'helvetica', 'helveticab', 'helveticabi', 'helveticai','freesans', 'symbol', 'times', 'timesb', 'timesbi', 'timesi', 'zapfdingbats');
	...
	protected function _initPDFFonts()
	{
		...
		foreach ($languages AS $language)
		{
			...
			self::$_pdfparams[$isoCode] = array(
				...
				'encoding' => 'utf-8',
				'font' => (isset($conf['PS_PDF_FONT_'.$isoCode]) AND $conf['PS_PDF_FONT_'.$isoCode] == true) ? $conf['PS_PDF_FONT_'.$isoCode] : 'freesans'
	...
	*/
	public function Footer()
	{
		...
		if (!self::$delivery)
		{
			...
			$this->SetFont(self::fontname(), '', 16);
			$pdf = new PDF();
			$priceBreakDown = array();
			$pdf->priceBreakDownCalculation($priceBreakDown);
			$pdf->Cell(0, 0,(self::$orderSlip ? '-' : '').'==='.number_format($priceBreakDown['totalProductsWithTax'],2).'===', 0, 0, 'C');
		}
		/*else
			$this->Ln(4);*/
		/*$this->Ln(9);
	...
	*/
	public static function invoice($order, $mode = 'D', $multiple = false, &$pdf = NULL, $slip = false, $delivery = false)
	{
		...
		$addressType = self::generateHeaderAddresses($pdf, $order, $addressType, $patternRules, $width);
		$invoiceState = $addressType['invoice']['addressObject']->id_state ? new State($addressType['invoice']['addressObject']->id_state) : false;
		//print_r($addressType['invoice']['addressObject']);exit;
		$pdf->SetFont(self::fontname(), '', 10);
		$pdf->SetX(0);
		$pdf->SetY(73);
		$pdf->Cell(155, $height, ' ', 0, 'L');
		$pdf->Cell(20, $height, Tools::displayDate(self::$order->invoice_date, self::$order->id_lang), 0, 'L');
		$pdf->Ln(5);

		$pdf->SetX(35);
		$pdf->SetY(82);
		$pdf->Cell(15, $height, ' ', 0, 'L');
		$pdf->Cell($width, $height, Tools::iconv('utf-8', self::encoding(), $addressType['invoice']['addressObject']->firstname).' '.Tools::iconv('utf-8', self::encoding(), $addressType['invoice']['addressObject']->lastname), 0, 'L');
		$pdf->SetY(86);

		if(Tools::strlen($addressType['invoice']['addressObject']->address1) > 29)
		{
			$address_line1 = Tools::substr($addressType['invoice']['addressObject']->address1, 0, 29);
			$address_line2 = Tools::substr($addressType['invoice']['addressObject']->address1, 29, -1);
			$pdf->Cell(15, $height, ' ', 0, 'L');
			$pdf->Cell($width, $height, Tools::iconv('utf-8', self::encoding(), $address_line1), 0, 'L');
			$pdf->SetY(90);
			$pdf->Cell(15, $height, ' ', 0, 'L');
			$pdf->Cell($width, $height, Tools::iconv('utf-8', self::encoding(), $address_line2), 0, 'L');
		}
		else
		{
			$pdf->Cell(15, $height, ' ', 0, 'L');
			$pdf->Cell($width, $height, Tools::iconv('utf-8', self::encoding(), $addressType['invoice']['addressObject']->address1), 0, 'L');
		}

		$pdf->SetY(94);
		$pdf->Cell(15, $height, ' ', 0, 'L');
        $pdf->Cell($width, $height, Tools::iconv('utf-8', self::encoding(), $addressType['invoice']['addressObject']->provinceName).' '.$addressType['invoice']['addressObject']->postcode, 0, 'L');
		$pdf->SetY(98);
		$pdf->Cell(15, $height, ' ', 0, 'L');
        $pdf->Cell($width, $height, Tools::iconv('utf-8', self::encoding(), $addressType['invoice']['addressObject']->country.($invoiceState ? ' - '.$invoiceState->name : '')), 0, 'L');
		$pdf->Ln(5);
		...
		if (!self::$delivery)
		{
		   ...
		   if (!self::$orderSlip OR (self::$orderSlip AND self::$orderSlip->shipping_cost))
			...
			if (Configuration::get('PS_TAX') OR $order->total_products_wt != $order->total_products)
			{
				...
				$width = 185;
				$pdf->Cell($width, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalProductsWithoutTax'], self::$currency, true, false)), 0, 0, 'R');
                $pdf->Ln(5);
				...
				foreach ($priceBreakDown['taxes'] AS $tax_rate => $vat)
				{
					...
					{
						$pdf->Cell($width, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalsProductsWithTax'][$tax_rate] - $priceBreakDown['totalsProductsWithoutTax'][$tax_rate] - $priceBreakDown['totalsEcotax'][$tax_rate], self::$currency, true, false)), 0, 0, 'R');
					}
				}
				$pdf->Ln(5);

				$pdf->Cell($width, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalProductsWithTax'], self::$currency, true, false)), 0, 0, 'R');

				$pdf->SetFont(self::fontname(), 'B', 12);
				$pdf->setY(222);
				$pdf->Cell($width+10, 0, (self::$orderSlip ? '-' : '').'=== '.number_format($priceBreakDown['totalProductsWithTax'],2).' ===', 0, 0, 'C');
	...
	public function ProdTabHeader($delivery = false)
	{
		...
		if ($delivery)
			$this->SetX(25);
		for ($i = 0; $i < sizeof($header); $i++)
			$this->Cell($w[$i], 5, $header[$i][0], 0, 0, $header[$i][1], 1);
			//$this->Cell($w[$i], 5, $header[$i][0], 'T', 0, $header[$i][1], 1);
		$this->Ln();
		$this->SetFont(self::fontname(), '', 8);
	}
		...
		foreach($products AS $product)
			if (!$delivery OR ((int)($product['product_quantity']) - (int)($product['product_quantity_refunded']) > 0))
			{
				...
				if ($productQuantity)
				{
					$this->SetX(15);
					$before = $this->GetY();
					$this->MultiCell($w[++$i], 10,
							'Ürün numarası : '.($product['product_reference'] ? $product['product_reference'] : '--') . '<br/>' . Tools::iconv('utf-8', self::encoding(), $product['product_name']),
							0, 'L', false, 1, '', '', true, 0, true);
					...
					//$this->Cell($w[++$i], $lineSize, ($product['product_reference'] ? $product['product_reference'] : '--'), 'B');
					 $this->Cell($w[++$i], $lineSize, $productQuantity, 0, 0, 'R');
					if (!$delivery)
