...
class CategoryCore extends ObjectModel
{
	...
	protected 	$fieldsRequiredLang = array('name', 'link_rewrite');
 	protected 	$fieldsSizeLang = array('name' => 64, 'link_rewrite' => 64, 'meta_title' => 128, 'meta_description' => 255, 'meta_keywords' => 255, 'header_text' => 255, 'header' => 128);
 	protected 	$fieldsValidateLang = array('name' => 'isCatalogName', 'link_rewrite' => 'isLinkRewrite', 'description' => 'isString',
											'meta_title' => 'isGenericName', 'meta_description' => 'isGenericName', 'meta_keywords' => 'isGenericName', 'header_text' => 'isGenericName', 'header' => 'isGenericName');
	...
	public function getTranslationsFieldsChild()
	{
		...
		//return parent::getTranslationsFields(array('name', 'description', 'link_rewrite', 'meta_title', 'meta_keywords', 'meta_description', 'header_text', 'header'));
		$fieldsArray = array('name', 'link_rewrite', 'meta_title', 'meta_keywords', 'meta_description', 'header_text', 'header');
		$fields = array();
	...
	function recurseLiteCategTree($maxDepth = 3, $currentDepth = 0, $id_lang = NULL, $excludedIdsArray = NULL)
	{
		...
		return array(
			...
			//'link' => $link->getCategoryLink((int)$this->id, $this->link_rewrite),
                        'link' => $link->getCategoryLink($this->link_rewrite),
			'name' => $this->name,
	...
	protected function recursiveDelete(&$toDelete, $id_category)
	{
	 	if (! is_array($toDelete) OR ! $id_category) {
	 		die(Tools::displayError('Error: 201406051428'));
	 	}
	...
	public static function getCategories($id_lang = false, $active = true, $order = true, $sql_filter = '', $sql_sort = '',$sql_limit = '')
	{
	 	if (! Validate::isBool($active)) {
	 		die(Tools::displayError('Invalid active flag: ' . $active));
	 	}
	...
	{
	 	if (! Validate::isBool($active)) {
	 		die(Tools::displayError('Invalid active flag passed: ' . $active));
	 	}
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT c.*, cl.id_lang, cl.name, cl.description, cl.link_rewrite, cl.meta_title, cl.meta_keywords, cl.meta_description, cl.header_text, cl.header
		FROM `'._DB_PREFIX_.'category` c
	...
	public function getProducts($id_lang, $p, $n, $orderBy = NULL, $orderWay = NULL, $getTotal = false, $active = true, $random = false, $randomNumberProducts = 1, $checkAccess = true, $diffColorQty = false)
	{
		global $cookie;
		...
		if (! Validate::isBool($active) OR ! Validate::isOrderBy($orderBy) OR ! Validate::isOrderWay($orderWay)) {
			die (Tools::displayError('Error: 201306051430'));
		}
		...
		if ($getTotal) {
			...
			WHERE cp.`id_category` = '.(int)($this->id).($active ? ' AND p.`active` = 1 AND p.`showing_status` = 1' : '').'
			'.($id_supplier ? 'AND p.id_supplier = '.(int)($id_supplier) : ''));
			return isset($result) ? $result['total'] : 0;
		}

		$data = Product::getAttributeGroup((int)($this->id));
        ...
        $resultIds = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS($sql);

        $result = array();
        foreach($resultIds as $productItem) {
            $product = Product::getProductByIdForFrontend($productItem['id_product'], $id_lang);
            $result[] = $product;
        }

		foreach($result as $key =>$res)
		{
			if(isset($res['category_default']) AND $res['category_default'] == 'Jewelry'){
				$result_sql= 'SELECT id_image  FROM `'._DB_PREFIX_.'image` WHERE id_product='.$res['id_product'].' AND position=(SELECT im.position+1 FROM `'._DB_PREFIX_.'image` im WHERE im.id_product='.$res['id_product'].' AND im.id_image='.$res['id_image'].')';
			}
			else{
				$result_sql= 'SELECT id_image  FROM `'._DB_PREFIX_.'image` WHERE id_product='.$res['id_product'].' AND position=(SELECT im.position+3 FROM `'._DB_PREFIX_.'image` im WHERE im.id_product='.$res['id_product'].' AND im.id_image='.$res['id_image'].')';
			}
			 $answer =Db::getInstance()->getRow($result_sql);
			 $result[$key]['mouseover_image'] = $res['id_product']."-".$answer['id_image'];
		}
...
//		return $link->getCategoryLink($this->id, $this->link_rewrite);
		return $link->getCategoryLink($this->link_rewrite);
	}
	...
	/*Get the parent category Id of subcategory.
	 *
	 * @param $langId is Language Id,
	 * @param $categoryName is Name of the category
	 * Return parent category Id.
	 */
	public function getParentCategoryIdByName($langId, $categoryName) {
		$result =  Db::getInstance()->getRow('
		SELECT c.`id_parent`
		FROM `'._DB_PREFIX_.'category` c
	    LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category` AND `id_lang` = '.(int)($langId).')
	    WHERE `name`  LIKE \''.pSQL($categoryName).'\'
		AND c.`id_category` != 1');
	...
	*/
	...
	static public function updateShowroomCategoryIds($update_ids, $customer_cat_style)
	{
		$where = 'category_name = \''.$customer_cat_style.'\'';
		return Db::getInstance()->AutoExecute(_DB_PREFIX_.'showroom_category_ids', $update_ids, 'UPDATE', $where );
	}

    static public function getCategoryByLinkRewrite($category_link_rewrite , $id_lang)
    {
        return Db::getInstance()->getRow('
		SELECT *
		FROM `'._DB_PREFIX_.'category_lang` cl
		LEFT JOIN  `'._DB_PREFIX_.'category` c ON cl.`id_category` = c.`id_category`
		WHERE cl.`link_rewrite` LIKE \''.pSQL($category_link_rewrite).'\' AND cl.id_lang = '.$id_lang
		);
    }

	public function getFilterProducts($id_lang, $p, $n, $orderBy = NULL, $orderWay = NULL, $getTotal = false, $active = true, $random = false, $randomNumberProducts = 1, $checkAccess = true, $diffColorQty = false, $filterList = null)
	{
		global $cookie;

		if($filterList == null || $filterList == '') {
			return Category::getProducts($id_lang, $p, $n,  NULL, NULL, false, true, false, 1, true, false);
		}
		...
		if (! Validate::isBool($active) OR ! Validate::isOrderBy($orderBy) OR ! Validate::isOrderWay($orderWay)) {
			die (Tools::displayError('Error: 201306051431'));
		}
		...
		$sql = 'SELECT p.*, pa.`id_product_attribute`,cp.`id_category` AS cat_id,IF(pa.default_image, p.`id_product`,\'  \')  AS product_combination, pl.`description`, pl.`description_short`,pl.collection_name, pl.`available_now`, pl.`available_later`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, i.`id_image`, il.`legend`, m.`name` AS manufacturer_name, IF(pa.`default_on`, pa.`id_product_attribute` ,\'  \') as default_combination, IF(pa.default_image, pa.`default_image` ,i.`id_image`) as id_image, tl.`name` AS tax_name, t.`rate`, cl.`name` AS category_default, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0 AS new,
	 (p.`price` * IF(t.`rate`,((100 + (t.`rate`))/100),1)) AS orderprice
			 ...
			 WHERE cp.`id_category` = '.(int)($this->id).($active ? ' AND p.`active` = 1 AND p.`showing_status` = 1' : '').'
			 '.($id_supplier ? 'AND p.id_supplier = '.(int)$id_supplier : '').'
			 AND p.`id_product` IN (SELECT DISTINCT p.`id_product`
									 FROM `bu_category_product` cp
									 LEFT JOIN `'._DB_PREFIX_.'product` p ON p.`id_product` = cp.`id_product`
									 ...
									 LEFT JOIN `'._DB_PREFIX_.'attribute_lang` al ON (al.`id_attribute` = a.`id_attribute` AND al.`id_lang` = 4)
									 WHERE cp.`id_category` = '.(int)($this->id).($active ? ' AND p.`active` = 1 AND p.`showing_status` = 1' : '').'
									 AND p.`active` = 1 AND p.`showing_status` = 1 AND al.name IN ('.$filterList.')) ';
		...
		/*foreach($result as $key =>$res)
		{
			 $result_sql= 'SELECT id_image  FROM `'._DB_PREFIX_.'image` WHERE id_product='.$res['id_product'].' AND position=(SELECT im.position+2 FROM `'._DB_PREFIX_.'image` im WHERE im.id_product='.$res['id_product'].' AND im.id_image='.$res['id_image'].')';
			 $answer =Db::getInstance()->getRow($result_sql);
			 $result[$key]['mouseover_image'] = $res['id_product']."-".$answer['id_image'];
	...
	public function getSizeTagFilterProducts($id_lang, $p, $n, $orderBy = NULL, $orderWay = NULL, $getTotal = false, $active = true, $random = false, $randomNumberProducts = 1, $checkAccess = true, $diffColorQty = false, $tagFilterList = NULL, $sizeFilterList = NULL)
    {
        global $cookie;

        $filteredProductsByTags = array();
        $filteredProductsBySize = array();
        $filteredProductsIds = array();
        ...
        if($tagFilterList == null && $tagFilterList == '') {
            $tags = false;
		}
        if($sizeFilterList == null && $sizeFilterList == ''){
            $size = false;
		}
        ...
        if (! Validate::isBool($active) OR ! Validate::isOrderBy($orderBy) OR ! Validate::isOrderWay($orderWay)) {
            die(Tools::displayError('Error: 201306051432'));
        }
        ...
        if(!empty($sizeFilterList)) {
            $addedJoins .= 'LEFT JOIN `'._DB_PREFIX_.'product_attribute` pa ON (p.`id_product` = pa.`id_product` AND pa.active = 1 AND pa.`quantity` >= 1 )
            LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pac on (pac.`id_product_attribute` = pa.`id_product_attribute`)
            LEFT JOIN `'._DB_PREFIX_.'attribute` a on (pac.`id_attribute` = a.`id_attribute`)
            LEFT JOIN `'._DB_PREFIX_.'attribute_lang` al ON (al.`id_attribute` = a.`id_attribute` AND al.`id_lang` = 4)';

            $whereClauses .= ' AND al.name IN ('. $sizeFilterList.') ';
        }

        if (!empty($tagFilterList)) {
            $addedJoins .='LEFT JOIN `'._DB_PREFIX_.'product_tag` pt ON (pt.`id_product` = cp.`id_product`)
                LEFT JOIN `'._DB_PREFIX_.'tag` t ON (t.`id_tag` = pt.`id_tag` AND t.`id_lang`='.$id_lang.')
                LEFT JOIN `'._DB_PREFIX_.'filter_tags` ft ON (ft.`id_tag` = t.`id_tag` AND ft.`id_lang`='.$id_lang.')';

            $whereClauses .= ' AND pt.id_tag IN (' . $tagFilterList . ') ';
        }
        ...
        $sql = 'SELECT p.`id_product`
            ...
            ' .$addedJoins . '
            WHERE cp.`id_category` = '.(int)($this->id).($active ? ' AND p.`active` = 1 AND p.`showing_status` = 1'  : '').'
            '.($id_supplier ? 'AND p.id_supplier = '.(int)$id_supplier : '') . $whereClauses;
        ...
        foreach($resultIds as $productItem) {
            $product = Product::getProductByIdForFrontend($productItem['id_product'], $id_lang);
            $result[] = Product::getProductProperties($id_lang, $product, $diffColorQty);
        }
