...
class ConfigurationCore extends ObjectModel
{
	...
	{
		if (! Validate::isConfigName($key)) {
	 		die(Tools::displayError('Given key cannot be found in Configuration table: ' . $key));
	 	}
	...
	public static function getMultiple($keys, $id_lang = NULL)
	{
	 	if (! is_array($keys) OR ! is_array(self::$_CONF) OR ($id_lang AND ! is_array(self::$_CONF_LANG))) {
	 		die(Tools::displayError('Error: 201306051435'));
	 	}
	...
	public static function updateValue($key, $values, $html = false)
	{
		...
		if (! Validate::isConfigName($key)) {
	 		die(Tools::displayError('Given key cannot be found in Configuration: ' . $key));
	 	}
	...
	public static function loadConfiguration()
    {
        ...
        BCache::$force = true;
        $result = BCache::get("butigoConfiguration");
        if (!$result) {
            ...
            Bcache::set("butigoConfiguration", $result, 3600);
        } else {
            Db::getInstance();
        }
