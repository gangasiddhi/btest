...
class CartCore extends ObjectModel
{
    ...
    public static function getTaxesAverageUsed($id_cart) {
        ...
        if (! Validate::isLoadedObject($cart)) {
            die(Tools::displayError('No cart found with cart id: ' . $id_cart));
        }
    ...
    public function getDiscounts($lite = false, $refresh = false)
    {
        ...
        /**********************TODO-CategoriesDiscount : Dont delete the below commented code
    ...
    public function getProducts($refresh = false, $id_product = false)
    {
        ...
        $sql = '
            SELECT IF(pa.default_image, p.`id_product`,\'  \')  AS product_combination,p.`id_color_default`, cp.`id_product_attribute`, cp.`id_product`, cu.`id_customization`, cp.`quantity` AS cart_quantity, cu.`quantity` AS customization_quantity, pl.`name`,
        ...
        foreach ($result AS $row) {
            ...
            if ($multiple_color = $this->getMultipleColorProduct($row['id_product'])) {
                $result2 = Db::getInstance()->ExecuteS('
                    SELECT pa2.`id_product_attribute`,pa2.`default_image`,pac2.`id_attribute`
                    ...
                    WHERE pa2.`id_product` = '.$row['id_product'].' AND  a2.`id_attribute_group` = 2');
                ...
                foreach($result2 as $res) {
                    ...
                    if ($res['default_image'] != 0) {
                        ...
                        $test[$res['id_attribute']]['default_ipa']=$res['id_product_attribute'];
                    }
                }

                foreach($test as $key => $t) {
                    if (in_array($row['id_product_attribute'],$t)) {
                        $id_image=$t['default_image'];
                        $row['default_ipa_only'] = $t['default_ipa'];
                    }
                }
            } else {
                unset($test);
            ...
            $row['isAccessoriesRelatedProduct'] = in_array($row['id_product_type'], array(
                Configuration::get('PROD_TYPE_ACCESSORIESED_PRODUCT'),
                Configuration::get('PROD_TYPE_PRODUCT_ACCESSORY')
            ));
    ...
    public  function updateQty($quantity, $id_product, $id_product_attribute = NULL, $id_customization = false, $operator = 'up')
    {
        ...
        if (! Validate::isLoadedObject($product)) {
            die(Tools::displayError('No product found with product id: ' . $id_product));
        }
    ...
    */
    ...
    public function _addCustomization($id_product, $id_product_attribute, $files, $textFields, $quantity)
    {
        if (! is_array($files) OR ! is_array($textFields)) {
            die(Tools::displayError('Error: 201306051424'));
        }
    ...
    */
    ...
    public static function getTotalCart($id_cart, $use_tax_display = false)
    {
        ...
        if (! Validate::isLoadedObject($cart)) {
            die(Tools::displayError('No cart is found with cart id: ' . $id_cart));
        }
    ...
    */
    public function getOrderTotal($withTaxes = true, $type = Cart::BOTH)
    {
        ...
        if (! in_array($type, array(Cart::ONLY_PRODUCTS, Cart::ONLY_DISCOUNTS, Cart::BOTH, Cart::BOTH_WITHOUT_SHIPPING, Cart::ONLY_SHIPPING, Cart::ONLY_WRAPPING, Cart::ONLY_PRODUCTS_WITHOUT_SHIPPING))) {
            die(Tools::displayError('Invalid type [' . $type . '] for order..'));
        }
        ...
        if ($type != Cart::ONLY_PRODUCTS)
        {
            ...
            if ($discountIds = $this->getDiscounts(true))
            {
                foreach ($discountIds AS $id_discount)
                {
                    ...
                    {
                        $discounts[] = $discount;
                        if ($discount->id_discount_type == 3 || $discount->id_discount_type == _PS_OS_CREDIT_ID_TYPE_)
                            foreach($products AS $product)
                            {
                                /**********************TODO-CategoriesDiscount : Dont delete the below commented code
                                * Please uncomment the below code when you are using the Categories
                ...
                foreach ($discounts AS $discount)
                    if ($discount->id_discount_type != 3)
                    {
                        ...
                        if (in_array($discount->id_discount_type, array(2, 7)))
...
//                              if (Product::idIsOnCategoryId((int)($product['id_product']), $categories))
    ...
    */
    function checkDiscountValidity($discountObj, $discounts, $order_total, $products, $checkCartDiscount = false) {
        ...
        if (in_array($discountObj->id_discount_type, array(2, 7)) AND $this->id_currency != $discountObj->id_currency) {
            return Tools::displayError('This voucher can only be used in the following currency:')
                . Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT `name` FROM `'._DB_PREFIX_.'currency` WHERE id_currency = '
                . (int) $discountObj->id_currency);
        }
...
//CD                }
    ...
    */
    static function getNonOrderedCarts($dateFrom, $dateTo) {
        ...
        if (! Validate::isDate($dateFrom) OR ! Validate::isDate($dateTo)) {
            die(Tools::displayError('Invalid dates have been passed: ' . $dateFrom . ' - ' . $dateTo));
        }
    ...
    */
    ...
    public static function getCustomerCarts($id_customer, $pagination = false, $limit = 10, $page = 1) {
        $sql =  'SELECT '.($pagination ? 'SQL_CALC_FOUND_ROWS ' : '').' *
            FROM '._DB_PREFIX_.'cart c
    ...
    public function duplicate()
    {
        ...
        foreach($customsById as $customizationId => $val)
        {
            ...
            $custom_ids[$custom['id_customization']] = Db::getInstance()->Insert_ID();
        }
    ...
    public function updateDiscountValueInCart($id_discount = 0, $price_return = false)
    {
		...
		if($this->nbProducts() > 1) {
			foreach($cart_products as $cart_product) {
				$cart_prices[$cart_product['id_product']] = $cart_product['price_wt'];
				$cart_qtys[$cart_product['id_product']][$cart_product['id_product_attribute']] = $cart_product['cart_quantity'];
				$cart_price[] = $cart_product['price_wt'];
			}

			$cart_qty = array();
			foreach($cart_qtys  as $key => $quantity) {
				foreach($quantity as $qty) {
					if(sizeof($qty) > 1) {
						$cartqty=0;
						$cartqty += $qty;
						$cart_qty[$key] = $cartqty;
					} else {
						$cart_qty[$key] = $qty;
					}
				}
			...
			if(sizeof($cart_prices) > 1) {
				asort($cart_prices);
			}

			$half_cart  = $this->nbProducts()/2;

			if(is_float($half_cart)) {
				$no_of_free_items = floor($half_cart);
			} else {
				$no_of_free_items = $half_cart;
			}
			...
			if(sizeof($cart_prices) > 1) {
				foreach($cart_prices AS $key => $cart_price) {
					if($counter == 0) {
						if( $cart_qty[$key] == $no_of_free_items || $cart_qty[$key] > $no_of_free_items ) {
							$price = $cart_price *  $no_of_free_items;
							$flag = true;
						}
					}
					if($flag == true)
						...
						if($no_of_free_items != 0) {
							if($no_of_free_items > 1) {
								$price += ($cart_price * $cart_qty[$key]);
								$no_of_free_items -=  $cart_qty[$key];
							}
							else {
								$price += ($cart_price * $no_of_free_items);
								$no_of_free_items--;
				...
				$price =  $cart_price[0] * $no_of_free_items;
			}
		}
