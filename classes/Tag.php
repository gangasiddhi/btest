...
class TagCore extends ObjectModel
{
	...
	*/
	...
	public static function deleteTagsForProduct($id_product)
	{
        ...
        public static function addSingleTag($name , $id_lang)
        {
            ...
            if (!Validate::isGenericName($name))
                    return false;
            $tagObj = new Tag(NULL, trim($name), (int)($id_lang));

            /* Tag does not exist in database */
            if (!Validate::isLoadedObject($tagObj))
            {
                    $tagObj->name = trim($name);
                    $tagObj->id_lang = (int)($id_lang);
                    if($tagObj->add())
                    {
                        $return = array('id' => $tagObj->id ,'name' => $tagObj->name);
                        return $return;
                    }
                    else
                        return 'Tag could not be added to the databse'; 
            }
            else
                return 'Tag already exists'; 
        }
        
//        public static function tagExists($name, $id_lang)
//        {
...
//        }   
        ...
        public static function getEachProductTags($id_product, $id_lang)
	{
                if (!$tmp = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT t.`id_tag`, t.`id_lang`, t.`name` 
		FROM '._DB_PREFIX_.'tag t 
		LEFT JOIN '._DB_PREFIX_.'product_tag pt ON (pt.id_tag = t.id_tag) 
		WHERE pt.`id_product`='.(int)($id_product).' AND t.id_lang = '.(int)($id_lang)))
	 	...
	 	foreach ($tmp AS $tag)
	 	{	
                    $result[$tag['id_tag']] = $tag['name'];
                }
	 	return $result;
	}
        
        public static function getAvailableTagsForProduct($id_product, $id_lang)
        {
            $tag_list = '';
            $products_tags = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
            SELECT id_tag
            ...
            SELECT *
            FROM `'._DB_PREFIX_.'tag` 
            WHERE id_lang = '.(int)($id_lang).' '.($tag_list ? 'AND id_tag NOT IN  ('.$tag_list.')' : $tag_list).' ORDER BY name');
        }
        
        public static function addProductTags($id_product, $tag_array)
        {
            $result = self::deleteTagsForProduct($id_product);
