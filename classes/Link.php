...
class LinkCore
{
	...
	public function getProductLink($id_product, $id_product_attribute = NULL, $alias = NULL, $category = NULL, $ean13 = NULL, $id_lang = NULL)
	{
		if (is_object($id_product))
			return ($this->allow == 1)?(_PS_BASE_URL_.__PS_BASE_URI__.$this->getLangLink((int)$id_lang).((isset($id_product->category) AND !empty($id_product->category) AND $id_product->category != 'home') ? $id_product->category.'/' : '').(int)$id_product->id.($id_product_attribute ? '-'.(int)$id_product_attribute : '').'-'.$id_product->link_rewrite.($id_product->ean13 ? '-'.$id_product->ean13 : '').'.html') :
			(_PS_BASE_URL_.__PS_BASE_URI__.'product.php?id_product='.(int)$id_product->id.'&id_product_attribute='.(int)$id_product_attribute);
		elseif ($alias)
			return ($this->allow == 1)?(_PS_BASE_URL_.__PS_BASE_URI__.$this->getLangLink((int)$id_lang).(($category AND $category != 'home') ? ($category.'/') : '').(int)$id_product.($id_product_attribute ? '-'.(int)$id_product_attribute : '').'-'.$alias.($ean13 ? '-'.$ean13 : '').'.html') :
			(_PS_BASE_URL_.__PS_BASE_URI__.'product.php?id_product='.(int)$id_product.'&id_product_attribute='.(int)$id_product_attribute);
		else
			return _PS_BASE_URL_.__PS_BASE_URI__.'product.php?id_product='.(int)$id_product.'&id_product_attribute='.(int)$id_product_attribute;
	}


//	public function getCategoryLink($id_category, $category_type = NULL , $alias = NULL, $id_lang = NULL)
//	{
////		print_r($id_category);echo "<br/>";
////		echo 'vibha'.$category_type;exit;
//		if (is_object($id_category))
//			return ($this->allow == 1) ? (_PS_BASE_URL_.__PS_BASE_URI__.$this->getLangLink((int)($id_lang)).(int)($id_category->id).($category_type ? '-'.$category_type : '').'-'.$id_category->link_rewrite) :
//			(_PS_BASE_URL_.__PS_BASE_URI__.'category.php?id_category='.(int)($id_category->id).'&shop_by='.$category_type);
//		if ($alias)
//			return ($this->allow == 1) ? (_PS_BASE_URL_.__PS_BASE_URI__.$this->getLangLink((int)($id_lang)).(int)($id_category).($category_type ? '-'.$category_type : '').'-'.$alias) :
//			(_PS_BASE_URL_.__PS_BASE_URI__.'category.php?id_category='.(int)($id_category).'&shop_by='.$category_type);
//		return _PS_BASE_URL_.__PS_BASE_URI__.'category.php?id_category='.(int)($id_category).'&shop_by='.$category_type;
//	}
        
        public function getCategoryLink($category_link_rewrite)
	{
                return ($this->allow == 1) ? (_PS_BASE_URL_.__PS_BASE_URI__.($category_link_rewrite)) :
			(_PS_BASE_URL_.__PS_BASE_URI__.'category.php?shop_by='.($category_link_rewrite));
		return _PS_BASE_URL_.__PS_BASE_URI__.'category.php?shop_by='.($category_link_rewrite);
	}
