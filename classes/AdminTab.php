...
abstract class AdminTabCore
{
	...
	public function __construct()
	{
		...
		$this->_conf = array(
			...
			26 => $this->l("Order state changed. Waiting for manager approval"),
			27 => $this->l("Product marked as waiting for manager approval"),
			28 => $this->l("Operation was successful")
		);
	 ...
	 */
	public function displayConf($msg = null) {
		$conf = Tools::getValue('conf', $msg);
		...
		if ($this->_list)
		{
			...
			foreach ($this->_list AS $tr)
			{
				...
				foreach ($this->fieldsDisplay AS $key => $params)
				{
					...
					elseif(isset($params['showing_active']) AND isset($tr[$key]))
						$this->_displayEnableLink($token, $id, $tr[$key], $params['showing_active'], Tools::getValue('id_category'), Tools::getValue('id_product'));
					...
					elseif (isset($tr[$key]))
					{
						$echo = ($key == 'price' ? round($tr[$key], 2) : isset($params['maxlength']) ? Tools::substr($tr[$key], 0, $params['maxlength']).'...' : (strcmp(strtolower($tr[$key]), strtolower("Kapıda Ödeme"))==0) ? "<span style='color:black'><b>".$tr[$key]."</b></span>" : $tr[$key]);
				...
				if ($this->edit OR $this->delete OR ($this->view AND $this->view !== 'noActionColumn') OR $this->status)
				{
	...
	protected function _displayStatus($token = NULL, $id)
	{
		global $currentIndex;

		echo '  <input type="checkbox" name="OrderId[]" value="'.$id.'" />';
