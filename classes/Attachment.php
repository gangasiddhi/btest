...
class AttachmentCore extends ObjectModel
{
	...
	protected	$fieldsValidate = array('file' => 'isGenericName','file1' => 'isGenericName','file2' => 'isGenericName', 'mime' => 'isCleanHtml', 'file_name' => 'isGenericName');

	protected	$fieldsRequiredLang = array('name');
	protected	$fieldsSizeLang = array('name' => 32);
	protected	$fieldsValidateLang = array('name' => 'isGenericName', 'description' => 'isCleanHtml' , 'quote' => 'isCleanHtml');
	...
	public function getFields()
	{
		...
		$fields['file1'] = pSQL($this->file1);
		$fields['file2'] = pSQL($this->file2);
	...
	public function getTranslationsFieldsChild()
	{
		...
		return parent::getTranslationsFields(array('name', 'description','quote'));
	}
	...
	public static function getStylist($id_lang,$num)
	{
		return Db::getInstance()->ExecuteS('
		SELECT *
		FROM '._DB_PREFIX_.'attachment a
		LEFT JOIN '._DB_PREFIX_.'attachment_lang al ON (a.id_attachment = al.id_attachment AND al.id_lang = '.intval($id_lang).')
		WHERE 	a.id_attachment = '.intval($num).'
		');
	}
	public static function listStylistIds()
	{
		$result =  Db::getInstance()->ExecuteS('
		SELECT *
		FROM '._DB_PREFIX_.'attachment a');
		foreach($result as $id)
			$styleid[] = $id['id_attachment'];
		return $styleid;
	}
