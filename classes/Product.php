...
class ProductCore extends ObjectModel
{
	...
	/** @var boolean Showing statuts */
	public		$showing_status = 1;
	...
	/**@var integer type of product*/
	public      $id_product_type;

	/**@var varchar json_encode data*/
	...
	/*** @var string Object date of last publish */
	public 	$date_of_last_publish;

	public 		$wont_be_sold_again_this_year;
	public 		$will_be_sold_again_next_year;
	public 		$wont_be_sold_again;
	...
	protected $fieldsValidate = array(
		...
		'design_quality_score' => 'isUnsignedInt',
		'on_sale' => 'isBool',
		'online_only' => 'isBool',
		'ecotax' => 'isPrice',
		'unit_price' => 'isPrice',
		...
		'date_of_last_publish' => 'isDate',
		'wont_be_sold_again_this_year' => 'isBool',
		'will_be_sold_again_next_year' => 'isBool',
		'wont_be_sold_again' => 'isBool'
	);
	...
	protected $fieldsValidateLang = array(
		...
		'meta_title' => 'isGenericName', 'link_rewrite' => 'isLinkRewrite', 'name' => 'isCatalogName', 'collection_name' => 'isCatalogName',
    ...
    public static function invalidateCacheById($id, $id_lang = 4)
    {
        BCache::$force = true;
        BCache::remove("product_list_" . $id . "_". $id_lang);
        BCache::remove("product_detail_" . $id . "_". $id_lang);
        BCache::remove("product_property_" . $id . "_" . $id_lang);
    }

	public static function getIdByAttributeId($id_product_attribute) {
		$sql = 'SELECT bpa.`id_product`
			FROM `bu_product_attribute` bpa
			WHERE bpa.`id_product_attribute` = ' . pSQL($id_product_attribute);
	...
	public function getFields()
	{
		...
		$fields['design_quality_score'] = empty($this->design_quality_score) ? 0 : (int) $this->design_quality_score;
		$fields['on_sale'] = (int)($this->on_sale);
		$fields['online_only'] = (int)($this->online_only);
		$fields['ecotax'] = (float)($this->ecotax);
		$fields['unity'] = empty($this->unity) ? 0 : pSQL($this->unity);
		...
		$fields['showing_status'] = (int)($this->showing_status);
		...
		$fields['id_product_type'] = (int) $this->id_product_type;
		$fields['recommendations'] = $this->recommendations;
		...
		$fields['date_of_last_publish'] = empty($this->date_of_last_publish) ? NULL : pSQL($this->date_of_last_publish);
		$fields['wont_be_sold_again_this_year'] = empty($this->wont_be_sold_again_this_year) ? false : pSQL($this->wont_be_sold_again_this_year);
		$fields['will_be_sold_again_next_year'] = empty($this->will_be_sold_again_next_year) ? false : pSQL($this->will_be_sold_again_next_year);
		$fields['wont_be_sold_again'] = empty($this->wont_be_sold_again) ? false : pSQL($this->wont_be_sold_again);

		return $fields;
	}

	/**
	 * Overriding function to set NULL for empty `date_of_last_publish` field
	 */
	public function save($nullValues = false, $autodate = true) {
		return parent::save(true, $autodate);
	}
	...
	public function toggleStatus() {
	 	if (! Validate::isTableOrIdentifier($this->identifier) OR ! Validate::isTableOrIdentifier($this->table)) {
	 		die(Tools::displayError('Error: 201306051505'));
	 	} elseif (! key_exists('active', $this)) {
	 		die(Tools::displayError('Error: 201306051506'));
	 	}
		...
		if($this->active == 0 AND $this->showing_status == 1)
		{
			$this->showing_status = 0;
			$result = Db::getInstance()->Execute('
				UPDATE `' . pSQL(_DB_PREFIX_ . $this->table) . '`
				SET `active` = !`active`, `showing_status` = 0
				WHERE `' . pSQL($this->identifier) . '` = ' . (int)($this->id));

			Module::hookExec('updateProduct', array('product' => $this));
	...
	/*Toggle status function showing status*/
	public function showingtogglestatus()
	{
		if (! Validate::isTableOrIdentifier($this->identifier) OR ! Validate::isTableOrIdentifier($this->table)) {
	 		die(Tools::displayError('Error: 201306051505'));
	 	} elseif (! key_exists('active', $this)) {
		...
		$this->showig_status = (int)(! $this->showig_status);

		$result = Db::getInstance()->Execute('
			UPDATE `' . pSQL(_DB_PREFIX_ . $this->table) . '`
			SET `showing_status` = !`showing_status`
			WHERE `' . pSQL($this->identifier) . '` = ' . (int)($this->id));
	...
	*/
	public function getTranslationsFieldsChild()
	{
		...
		$fieldsArray = array('meta_description', 'meta_keywords', 'meta_title', 'link_rewrite', 'name','collection_name', 'available_now', 'available_later');
		$fields = array();
		...
		foreach ($languages as $language)
		{
			...
			$fields[$language['id_lang']]['collection_name'] = (isset($this->collection_name[$language['id_lang']])) ? pSQL($this->collection_name[$language['id_lang']], true) : '';
			foreach ($fieldsArray as $field)
			{
				if (!Validate::isTableOrIdentifier($field))
					die(Tools::displayError('Error: 201306051507'));
	...
	public static function initPricesComputation($id_customer = NULL)
	{
		...
		if ($id_customer)
		{
			...
			if (!Validate::isLoadedObject($customer))
				die(Tools::displayError('Error: 201306051508'));
	...
	static public function reorderPositions($id_category)
	{
		$result = Db::getInstance()->ExecuteS('
		SELECT `id_product`
		FROM `'._DB_PREFIX_.'category_product`
		...
		foreach($result as $res)
			$productids[] = $res['id_product'];
		//shuffle the array $productids
		shuffle($productids);
		//get the keys of the array $productids
		$positions = array_keys($productids);
		...
		for ($i = 0; $i < $sizeof;$i++)
		{
			...
			UPDATE `'._DB_PREFIX_.'category_product`
			SET `position` = '.intval($positions[$i]).'
			WHERE `id_category` = '.intval($id_category).'
			AND `id_product` = '.intval($productids[$i]));
		}
		return true;
	}
	...
	*/
	...
	public function validateFieldsLang($die = true, $errorReturn = false)
	{
		...
		foreach ($this->description_short as $k => $value)
			...
			{
				if ($die) die (Tools::displayError('Error: 201306051509').' ('.get_class($this).'->description: length > 400 for language '.$k.')');
	...
	public static function getByReference($reference)
	{
		...
		if (!Validate::isReference($reference))
			die(Tools::displayError('Error: 201306051510'));
    ...
    /**
     * Unique item cache which used in frontend like category panel
     */
    public static function getProductByIdForFrontend($id, $id_lang)
    {
        $key = "product_list_" . $id . "_". $id_lang;
        $cache = BCache::get($key);
        if (!$cache) {
            $sql = 'SELECT pa.`id_product_attribute`,
                    cp.`id_category` AS cat_id,
                    IF(pa.default_image, p.`id_product`,\'  \')  AS product_combination,
                    p.*, pl.`description`,
                    ...
                    pl.`meta_title`,
                    pl.`name`,
                    IF(pa.`default_on`, pa.`id_product_attribute` ,\'  \') as default_combination,
                    IF(pa.default_image, pa.`default_image` ,i.`id_image`) as id_image ,
                    il.`legend`,
                    ...
                    (p.`price` * IF(t.`rate`,((100 + (t.`rate`))/100),1)) AS orderprice, cp.`position`
                FROM `'._DB_PREFIX_.'category_product` cp
	...
	{
		if (!Validate::isReference($reference))
			die(Tools::displayError('Error: 201306051511'));

		$result = Db::getInstance()->getRow('
		SELECT pa.`id_product`,pa.id_product_attribute,pa.quantity as prev_qty
	...
	/**
	 * Used for decreasing stock from warehouse stock information
	 */
	public static function getAllocatedProductsFromOpenOrders() {
		$openStatuses = array(
		    _PS_OS_PREPARATION_, // common to all
		    _PS_OS_WAITING_FOR_CUSTOMER_APPROVAL_, // COD first status
		    _PS_OS_PAYMENT_, // free order
		    _PS_OS_FIRST_ORDER_OF_CUSTOMER_, // moderation fraud check
		    _PS_OS_BACK_ORDER_ // back order
		);
		$os = join(', ', $openStatuses);
		$sql = "
			SELECT OrderId, RefCode, ProductQuantity
			FROM
			(
			    SELECT bo.`id_order` AS 'OrderId',
			        bod.`product_reference` AS 'RefCode',
			        bod.`product_quantity` AS 'ProductQuantity',
			        @LastStatus := (
			            SELECT boh.`id_order_state`
			            FROM `bu_order_history` boh
			            WHERE boh.`id_order` = bo.`id_order`
			            ORDER BY boh.`date_add` DESC
			            LIMIT 1
			        ) AS 'LastStatus'
			    FROM `bu_orders` bo
			    JOIN `bu_order_detail` bod ON bod.`id_order` = bo.`id_order`
			) DATA
			WHERE LastStatus IN (" . pSQL($os) . ")
			ORDER BY OrderId
		";
	...
	public static function UpdateXmlProductQuantity($product)
	{
            if (!is_array($product))
            die (Tools::displayError('Error: 201306051512'));

            $subtract_quan = 0;
            $now = time();
            $last10days = $now - (90 * 24 * 60 *60);
            $date_to = date('Y-m-d H:i:s', $last10days );

            $sql = '
            SELECT od.`id_order` , o.date_add , od.product_reference
            FROM `'._DB_PREFIX_.'order_detail` od
            LEFT JOIN `'._DB_PREFIX_.'orders` o ON (od.`id_order` = o.`id_order`)
            WHERE od.`product_reference` LIKE \'%'.pSQL($product['reference']).'%\'
            ...
            foreach($result as $res)
            {
                    $orderHistory = OrderHistory::getLastOrderState($res['id_order']);
                    $order = new Order($res['id_order']);

                    if($orderHistory->id == _PS_OS_PREPARATION_ || $orderHistory->id == Configuration::get('PS_OS_WAITING_FOR_CUSTOMER') || strtolower($order->module) === 'freeorder')
                    {
                            $result2 = Db::getInstance()->ExecuteS('
                            SELECT od.product_quantity ,od.id_order
                            FROM `'._DB_PREFIX_.'order_detail` od
                            WHERE od.`product_reference` LIKE \'%'.pSQL($res['product_reference']).'%\'
                            AND od.`id_order` = '.intval($res['id_order']).'
                            ');

                            foreach($result2 as $res2)
                            {
                                    $subtract_quan += $res2['product_quantity'];
                            }

                            $return['state'][] = $res['id_order'];
                    }
                    else
                            $return['otherstate'][] = $res['id_order'];
            }

            $qty = intval($product['quantity'] - $subtract_quan);
            $qty = $qty > 0 ? $qty : 0;
            $result3 = Db::getInstance()->Execute('
                    ...
                    AND `reference` LIKE \'%'.pSQL($product['reference']).'%\'
                    ');
            ...
            $return['product'][0] = $product['quantity'];
            $return['product'][1] = $subtract_quan;
            $return['product'][2] = $qty;
	...
	*/
	...
	public static function getAllProductIds($start = 100, $limit = 100, $order_by = 'date_add', $order_sort = 'ASC', $only_active = false) {
		$log = Logger::getLogger(get_class(self));
		$sql = 'SELECT bp.`id_product`
			FROM `bu_product` bp
			ORDER BY bp.`' . $order_by . '` ' . $order_sort . '
	...
	public static function getProductsWithAttribute() {
		$sql = "
	...
	*/
	public static function getProducts($id_lang, $start, $limit, $orderBy, $orderWay, $id_category = false, $only_active = false)
	{
		if (!Validate::isOrderBy($orderBy) OR !Validate::isOrderWay($orderWay))
			die (Tools::displayError('Error: 201306051513'));
	...
	public static function getAttributeGroup($id_category)
	{

		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT  a.`id_attribute_group`,cp.`id_product`
		FROM `'._DB_PREFIX_.'product_attribute` pa
		...
		where cp.`id_category` = '.intval($id_category).' AND ag.`is_color_group` = 1'
		);
		$data = array();
		$custom = array();$default= array();
		foreach($result as $res)
		{
			$data[$res['id_product']][] = $res['id_attribute_group'];
		}
		foreach($data as $key => $dat)
		{
			if(sizeof($dat) > 1)
				...
				$custom['no_color'][] = $key;
			//$final['count'][] = sizeof($dat);
		}
		//print_r($data);echo "<br/>";
		//print_r($custom);echo "<br/>";exit;
	...
	*/
	public function addProductAttribute($price, $weight, $unit_impact, $ecotax, $quantity, $id_images, $reference, $supplier_reference, $ean13, $default, $default_image = NULL, $location = NULL, $upc = NULL)
	{
		...
		'location' => pSQL($location), 'ean13' => pSQL($ean13), 'upc' => pSQL($upc), 'default_on' => (int)($default), 'default_image' => (int)($default_image[0])),
		'INSERT');
	...
	public function addCombinationEntity($wholesale_price, $price, $weight, $unit_impact, $ecotax, $quantity, $id_images, $reference, $supplier_reference, $ean13, $default, $default_image = NULL, $location = NULL, $upc = NULL)
	{
		if (
			!$id_product_attribute = $this->addProductAttribute($price, $weight, $unit_impact, $ecotax, $quantity, $id_images, $reference, $supplier_reference, $ean13, $default, $default_image, $location, $upc)
	...
	*/
	...
	/*
	 * update if the product combination is active or not.
	 */
	public function updateCombinationStatus($id_product_attribute, $status)
	{
		...
		UPDATE `'._DB_PREFIX_.'product_attribute`
		SET `active` = '.$status .'
		'.($result['default_on'] ? ',`default_on` = 0' : '').'
		'.($result['default_image'] ? ',`default_image` = null' : '').'
		WHERE `id_product` = '.(int)$this->id.'
		AND `id_product_attribute` = '.(int)$id_product_attribute.'');
	}
	...
	*/
	public function updateProductAttribute($id_product_attribute, $wholesale_price, $price, $weight, $unit, $ecotax, $quantity, $id_images, $default_image = NULL, $reference, $supplier_reference, $ean13, $default, $location = NULL, $upc = NULL, $minimal_quantity)
	{
		Db::getInstance()->Execute('
		...
		'default_image' =>	(int)($default_image[0]),
	...
	public function updateQuantityProductWithAttributeQuantity($color = false)
	{
		...
		(
			...
			FROM `'._DB_PREFIX_.'product_attribute`
			WHERE `id_product` = '.(int)($this->id).'
		), \'0\' ) '.($color ? ', `id_color_default` = 2' :',`id_color_default` = 0').'
		WHERE `id_product` = '.(int)($this->id));
	}
	/**
	...
	*/
	public function addAttributeCombinaison($id_product_attribute, $attributes)
	{
		if (!is_array($attributes))
			die(Tools::displayError('Error: 201306051514'));
		...
		if (!Validate::isValuesList($attributesList))
			die(Tools::displayError('Error: 201306051515'));
	...
	*/
	...
	public function getDefaultImage($id_lang) {
		global $link;
		...
		foreach ($images as $image) {
			if ($image['cover']) {
				$imageId = (Configuration::get('PS_LEGACY_IMAGES') ? $this->id . '-' . $image['id_image'] : $image['id_image']);
				$path = $link->getImageLink($this->link_rewrite[$id_lang], $imageId, 'large');

				$this->log->debug('Product Id: ' . $this->id . ', Image id: ' . $imageId . ', path: ' . $path);
	...
	{
		if (!$productAttributes = Db::getInstance()->ExecuteS('SELECT `id_product_attribute`,`default_image` FROM `'._DB_PREFIX_.'product_attribute` WHERE `id_product` = '.(int)($this->id)))
			return false;
		$ids = array();
		foreach ($productAttributes AS $productAttribute){
			$ids[] = (int)($productAttribute['id_product_attribute']);
			//if($productAttribute['default_image'])
			$multiplecolor[$productAttribute['id_product_attribute']] = $productAttribute['default_image'] ;
		}
...
$rows=array();
		...
		foreach ($multiplecolor AS $key => $productAttribute )
		{
			if($productAttribute>0)
				$images[$key]['coverimage'][] = $productAttribute ;
		}
	...
	*/
	public static function getNewProducts($id_lang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL)
	{
		...
		elseif ($orderBy == 'name')
			$orderByPrefix = 'pl';
		if (!Validate::isOrderBy($orderBy) OR !Validate::isOrderWay($orderWay))
			die(Tools::displayError('Error: 201306051516'));
	...
	*/
	public static function getPricesDrop($id_lang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL, $beginning = false, $ending = false)
	{
		if (!Validate::isBool($count))
			die(Tools::displayError('Error: 201306051517'));
		...
		elseif ($orderBy == 'name')
            $orderByPrefix = 'pl';
		if (!Validate::isOrderBy($orderBy) OR !Validate::isOrderWay($orderWay))
			die (Tools::displayError('Error: 201306051518'));
		$currentDate = date('Y-m-d H:i:s');
		$ids_product = self::_getProductIdByDate((!$beginning ? $currentDate : $beginning), (!$ending ? $currentDate : $ending));
	...
	*/
	public function	getImages($id_lang,$ipa = 0)
	{
//		echo '
...
//		echo 'SELECT IF(pa.default_image, pa.`default_image` ,i.`cover`) as cover, IF(pa.default_image, pa.`default_image` ,i.`id_image`) as id_image, il.`legend`, i.`position`
//		FROM `'._DB_PREFIX_.'image` i
//		LEFT JOIN `'._DB_PREFIX_.'product_attribute` pa ON (pa.`id_product` = i.`id_product`)
//		LEFT JOIN `'._DB_PREFIX_.'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = '.(int)($id_lang).')
//		WHERE i.`id_product` = '.(int)($this->id).($ipa>0 ? ' AND pa.`id_product_attribute`= '.(int)$ipa : '').'
//		ORDER BY `position`';

//		echo 'SELECT '.($ipa>0 ? ' pa.`default_image`' : 'i.`cover`').'as cover,
//			'.($ipa>0 ? ' pa.`default_image` ' : 'i.`id_image`').'as id_image, il.`legend`, i.`position`
//		FROM `'._DB_PREFIX_.'image` i
//		'.($ipa>0 ? 'LEFT JOIN `'._DB_PREFIX_.'product_attribute` pa ON (pa.`id_product` = i.`id_product`)' : '').'
//		LEFT JOIN `'._DB_PREFIX_.'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = '.(int)($id_lang).')
//		WHERE i.`id_product` = '.(int)($this->id).($ipa>0 ? ' AND pa.`id_product_attribute`= '.(int)$ipa : '').'
//		'.($ipa>0 ? 'GROUP BY pa.`id_product_attribute`' : 'ORDER BY `position`').'';
//
		return Db::getInstance()->ExecuteS('
		SELECT i.`cover`, i.`id_image`, il.`legend`, i.`position`
		FROM `'._DB_PREFIX_.'image` i
...
//	public function	getMultipleColorImages($id_lang,$ipa = 0)
//	{
//		return Db::getInstance()->ExecuteS('
//		SELECT pa.default_image as cover,pa.`id_product_attribute`
//		FROM`'._DB_PREFIX_.'product_attribute`
//		WHERE i.`id_product` = '.(int)($this->id).($ipa>0 ? ' AND pa.`id_product_attribute`= '.(int)$ipa : '').'
//		');
//	}
	...
	*/
	...
	$usereduc = true, $quantity = 1, $forceAssociatedTax = false, $id_customer = NULL, $id_cart = NULL, $id_address = NULL, &$specificPriceOutput = NULL, $with_ecotax = true, $use_groupReduction = true)
	{
		...
		if (!Validate::isBool($usetax) OR !Validate::isUnsignedId($id_product))
			die(Tools::displayError('Error: 201306051519'));
		// Initializations
		if (!$id_customer)
		...
		if (!is_object($cur_cart) OR (Validate::isUnsignedInt($id_cart) AND $id_cart))
		{
			...
			*/
			if (!$id_cart AND !Validate::isCookie($cookie))
				die(Tools::displayError('Error: 201306051520'));
	...
	*/
	...
	public static function getPriceWithoutReductStatic($id_product, $notax = false, $id_product_attribute = false)
	{
		return self::getPriceStatic((int)($id_product), !$notax, $id_product_attribute, 6, NULL, false, false);
	}
	...
	*/
	public static function getQuantity($id_product, $id_product_attribute = NULL, $cache_is_pack = NULL, $color = NULL, $diffColorQty = false)
	{
		...
		//if($color != NULL){
		//echo "d=".$diffColorQty;exit;
		if(($color != NULL) AND in_array($id_product,$color) AND $diffColorQty == true)
		{
			$ipasql = Db::getInstance()->ExecuteS('SELECT pac.`id_product_attribute`
						FROM `'._DB_PREFIX_.'product_attribute_combination` pac
						WHERE pac.`id_attribute`=(
						   SELECT pacn.`id_attribute`
						   FROM `'._DB_PREFIX_.'product_attribute_combination` pacn
						   LEFT JOIN `'._DB_PREFIX_.'attribute` AS a ON (a.`id_attribute` = pacn.`id_attribute`)
						   WHERE pacn.`id_product_attribute` = '.$id_product_attribute.' AND a.`id_attribute_group`=2
						   )');
			$ipa = array();
			foreach($ipasql as $ipavalue)
			{
				$ipa[] = $ipavalue['id_product_attribute'];
			}

			$res3 = Db::getInstance()->ExecuteS('SELECT SUM(pa.`quantity`) as total, pa.`id_product_attribute`
				FROM `'._DB_PREFIX_.'product` p
				LEFT JOIN `'._DB_PREFIX_.'product_attribute` AS pa ON pa.`id_product` = p.`id_product`
				LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` AS pac ON pa.`id_product_attribute` = pac.`id_product_attribute`
				LEFT JOIN `'._DB_PREFIX_.'attribute` AS a ON (a.`id_attribute` = pac.`id_attribute` )
				WHERE pa.`id_product` = '.(int)($id_product).' AND a.`id_attribute_group`= 2 GROUP BY pac.`id_attribute`');

			foreach($res3 as $res)
			{
				//if($res['id_product_attribute'] == $id_product_attribute)
				if(in_array($res['id_product_attribute'],$ipa))
		...
		else
		{
			...
			return Db::getInstance()->getValue($sqlresult);
		}
...
//		GROUP BY p.`id_product`');
	...
	*/
	...
	{
		if (!is_array($product))
			die (Tools::displayError('Error: 201306051521'));
	...
	public static function reinjectQuantities(&$orderDetail, $quantity)
	{
		...
		if (!Validate::isLoadedObject($orderDetail))
			die(Tools::displayError('Error: 201306051522'));
	...
	*/
	public function getAttributesGroups($id_lang)
	{
		...
		a.`color` AS attribute_color, pa.`id_product_attribute`, pa.`quantity`, pa.`price`, pa.`ecotax`, pa.`weight`, pa.`default_on`,pa.`default_image`, pa.`reference`, pa.`unit_price_impact`, pa.`minimal_quantity`
		FROM `'._DB_PREFIX_.'product_attribute` pa
		LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
		LEFT JOIN `'._DB_PREFIX_.'attribute` a ON a.`id_attribute` = pac.`id_attribute`
		LEFT JOIN `'._DB_PREFIX_.'attribute_group` ag ON ag.`id_attribute_group` = a.`id_attribute_group`
		...
		WHERE pa.`id_product` = '.(int)($this->id).' AND pa.`active` = 1
	...
	*/
	...
	{
		$result = Db::getInstance()->ExecuteS('
		SELECT p.`id_product`, pl.`name`, pl.`link_rewrite`, p.`weight`, p.`active`,p.`showing_status`, p.`ecotax`, i.`id_image`, p.`reference`, p.`cache_is_pack`,
	...
	*/
	...
	public static function getProductProperties($id_lang, $row, $diffColorQty = false)
    {
        $cache = false;

        //Caching disabled because we are facing some issues
        //if (!$diffColorQty) { //cache data if only !$diffColorQty
        //    $key = "product_property_" . $row['id_product'] . "_" . $id_lang;
        //    $cache = BCache::get($key);
        //    if (!$cache) {
        //        $cache = static::_getProductProperties($id_lang, $row, $diffColorQty = false);
        //        BCache::set($key, $cache, 0);
        //    }
        //}

        //if not cached then run normal query
        if (!$cache) {
            $cache = static::_getProductProperties($id_lang, $row, $diffColorQty = false);
        }
	...
	public static function _getProductProperties($id_lang, $row, $diffColorQty = false)
	{
		if (!$row['id_product'])
			return false;
		...
		$row['category'] = Category::getLinkRewrite((int)$row['id_category_default'], (int)($id_lang));
                ...
                if(isset($row['cat_id']))
                    $color = self::getAttributeGroup($row['cat_id']);
		if(isset($color['color']))
			$row['link'] = $link->getProductLink((int)$row['id_product'], (in_array($row['id_product'],$color['color'])?$row['id_product_attribute']:'') , $row['link_rewrite'], $row['category'], $row['ean13']);
		else
			$row['link'] = $link->getProductLink((int)$row['id_product'], NULL, $row['link_rewrite'], $row['category'], $row['ean13']);
		...
		$date1 = date('Y-m-d H:i:s'); //strftime('%c');
		$date2 = $specific_prices['to'];
		if($date2 > $date1)
		{
			$diff = abs(strtotime($date2) - strtotime($date1));
			if($diff > 0)
				$row['specific_time'] = $diff;
			else
				$row['specific_time'] = 0;
		}
		else
			$row['specific_time'] = 0;
		//echo "a=".$row['specific_time'];exit;

		if ($row['id_product_attribute'])
		{
			$row['quantity_all_versions'] = $row['quantity'];
			//echo "c=".$diffColorQty;
			$row['quantity'] = Product::getQuantity((int)$row['id_product'], $row['id_product_attribute'], isset($row['cache_is_pack']) ? $row['cache_is_pack'] : NULL, isset($color['color']) ? $color['color'] : NULL, $diffColorQty);
		}
		...
		if($result = Product::getColorsForShowroom($row['id_product'] ,(int)$id_lang ))
				$row['product_colors'] = $result;
		if($shoe_sizes = Product::getSingleColorShoeSizes( $row['id_product'], (int)$id_lang))
			$row['shoe_sizes'] = $shoe_sizes;
	...
	public static function getProductsProperties($id_lang, $query_result, $diffColorQty = false)
	{
		$resultsArray = array();
		foreach ($query_result AS $row)
			if ($row2 = Product::getProductProperties($id_lang, $row, $diffColorQty))
				$resultsArray[] = $row2;
		return $resultsArray;
	}
	...
	*/
	public static function getFrontFeaturesStatic($id_lang, $id_product)
	{
		if (!array_key_exists($id_product.'-'.$id_lang, self::$_frontFeaturesCache))
		{
			...
			LEFT JOIN '._DB_PREFIX_.'feature_lang fl ON (fl.id_feature = pf.id_feature AND fl.id_lang = '.(int)$id_lang.')
			LEFT JOIN '._DB_PREFIX_.'feature_value_lang fvl ON (fvl.id_feature_value = pf.id_feature_value AND fvl.id_lang = '.(int)$id_lang.')
			WHERE pf.id_product = '.(int)$id_product.' AND name NOT IN("Tanım","Menşei")
                        ORDER BY
                            Case pf.id_feature
                                when 11 then 7
                                when 7 then 11
	...
	*/
	...
	{
		$sql = 'SELECT id_product FROM `' . _DB_PREFIX_ . 'category_product` WHERE `id_product`=' . pSQL((int)($id_product));

		if (! empty($categories)) {
			$sql .=  ' AND `id_category` IN(';
	...
	public function addStockMvt($quantity, $id_reason, $id_product_attribute = NULL, $id_order = NULL, $id_employee = NULL)
	{
		...
		if ($stockMvtReason->sign == 1) {
			$this->log->debug('Increasing stock by ' . $qty);
			$this->quantity += $stock;
		} elseif ($stockMvtReason->sign == -1) {
			$this->log->debug('Decreasing stock by ' . $qty);
			$this->quantity -= $qty;
		}
	...
	*/
	...
	/*public static function getIpaAttribute($id_product)
	{
		echo '
		SELECT pac.`id_product_attribute` as ipa, pac.`id_attribute`
		FROM `'._DB_PREFIX_.'product_attribute_combination` pac
	...
	public static function getIdAttribute($id_product_attribute)
	{
		return Db::getInstance()->ExecuteS('
	 ...
	 public static function getColorsForShowroom($id_product , $id_lang)
	{
		$result =  Db::getInstance()->ExecuteS('
		SELECT ag.`id_attribute_group`, ag.`is_color_group`,  a.`id_attribute`, al.`name` AS attribute_name, a.`color` AS attribute_color
		FROM `'._DB_PREFIX_.'product_attribute` pa
		LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
		LEFT JOIN `'._DB_PREFIX_.'attribute` a ON a.`id_attribute` = pac.`id_attribute`
		LEFT JOIN `'._DB_PREFIX_.'attribute_group` ag ON ag.`id_attribute_group` = a.`id_attribute_group`
		LEFT JOIN `'._DB_PREFIX_.'attribute_lang` al ON a.`id_attribute` = al.`id_attribute`
		LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` agl ON ag.`id_attribute_group` = agl.`id_attribute_group`
		WHERE pa.`id_product` = '.(int)($id_product).' AND  ag.`is_color_group` = 1
	...
	public static function getSingleColorShoeSizes($id_product, $id_lang)
	{
        $query = '
            SELECT ag.`id_attribute_group`,  a.`id_attribute`, al.`name` AS attribute_name, pa.`quantity` AS product_qty, pa.`reference`
            FROM `'._DB_PREFIX_.'product_attribute` pa
            LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
            LEFT JOIN `'._DB_PREFIX_.'attribute` a ON a.`id_attribute` = pac.`id_attribute`
            LEFT JOIN `'._DB_PREFIX_.'attribute_group` ag ON ag.`id_attribute_group` = a.`id_attribute_group`
            LEFT JOIN `'._DB_PREFIX_.'attribute_lang` al ON (a.`id_attribute` = al.`id_attribute` AND al.`id_lang` = '.(int)($id_lang).')
            LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` agl ON ag.`id_attribute_group` = agl.`id_attribute_group`
            WHERE pa.`id_product` = '.(int)($id_product).' AND  agl.`id_attribute_group` = 4
            GROUP BY a.`id_attribute`
		';
		//echo "Query - $query <br>";
	 ...
	 * Return discount products_ids */
	public function getDiscountedProductslist()
	{
		global $cookie;
			$category_details = Category::searchByNameAndParentCategoryId((int)($cookie->id_lang), 'DSP', 1);
			...
			$discount_products = $category->getProducts((int)($cookie->id_lang), 1, $nb, 'position',NULL, false,true,false,1,true,true);

			if(!empty($discount_products))
			{
				foreach($discount_products as $discount_product)
					$discount_product_ids[] = $discount_product['id_product'];

				$discount_product_ids = array_unique($discount_product_ids);
	...
	/*Price Reduced product Ids list
	 * Returns the products ids of the price reduced products*/

	public function getPriceReducedProductList()
	{
		 $products = DB::getInstance()->ExecuteS('
			 SELECT sp.`id_product`, sp.`date_upd` FROM `'._DB_PREFIX_.'specific_price` sp');
		 ...
		 if(!empty($products))
			{
				foreach($products as $product)
					$price_reduced_product_ids[$product['id_product']] = $product['date_upd'];

				//$price_reduced_product_ids = array_unique($price_reduced_product_ids);

				return $price_reduced_product_ids;
			}
		else
			return array();
	}
	...
	static public function getProductTypes()
	{
		$product_types = DB::getInstance()->ExecuteS('
			 SELECT * FROM `'._DB_PREFIX_.'product_type`');

		return 	$product_types;
	}

	public function getFirstDayOfSale() {
		$sql = "SELECT MIN(bpi.`date_publish`)
            FROM `bu_product_publish_info` bpi
            WHERE bpi.`id_product` = " . $this->id;

		return DB::getInstance()->getValue($sql);
	}

	public function publish() {
		$this->date_of_last_publish = date('Y-m-d H:i:s');
	}

	public function unpublish() {
		if (! Validate::isLoadedObject($this) AND empty($this->date_of_last_publish))  {
			return false;
		}

		$publishDate = new DateTime($this->date_of_last_publish);
		$publishDateStr = $publishDate->format('Y-m-d H:i:s');
		$unpublishDate = new DateTime();
		$unpublishDateStr = $unpublishDate->format('Y-m-d H:i:s');
		$difference = intval($unpublishDate->diff($publishDate)->format("%a"));

		if ($difference == 0) {
			// don't insert this since it's not worth it and
			// causes false reports..

			// and also reset date_of_last_publish if necessary
			$firstDayOfSale = $this->getFirstDayOfSale();

			if (empty($firstDayOfSale)) {
				$this->date_of_last_publish = null;
			} else {
				$this->date_of_last_publish = $firstDayOfSale;
			}

			return false;
		}

		$sql = sprintf("INSERT INTO %s%s VALUES(%d, '%s', '%s', %d)", _DB_PREFIX_, 'product_publish_info', $this->id, $publishDateStr, $unpublishDateStr, $difference);
