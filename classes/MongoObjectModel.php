...
abstract class MongoObjectModelCore
{
	...
	/** @var string MongoDB Collectionidentifier */
	...
	public function __construct($id = NULL, $id_lang = NULL)
	{
		if ($id_lang != NULL && Validate::isLoadedObject(new Language($id_lang)))
	 	...
	 	/* Connect to database and check MongoDB collection /identifier */
	 	if (!Validate::isTableOrIdentifier($this->identifier) OR !Validate::isTableOrIdentifier($this->collection))
			die(Tools::displayError('Error: 201306051445'));
		$this->identifier = $this->identifier;

		/* Load object from database if object id is present */
		if ($id)
		{
			if (!isset(self::$_cache[$this->collection][(int)($id)][(int)($id_lang)]))
				self::$_cache[$this->collection][(int)($id)][(int)($id_lang)] = MongoDbApi::getInstance()->getRow(_DB_PREFIX_.$this->collection, array($this->identifier => (int)$id));

			$result = self::$_cache[$this->collection][(int)($id)][(int)($id_lang)];
			if ($result)
			{
				$this->id = (int)($id);
				...
				{
					$result = MongoDbApi::getInstance()->ExecuteS('
					SELECT *
					FROM `'.pSQL(_DB_PREFIX_.$this->collection).'_lang`
	...
	{
	 	if (!Validate::isTableOrIdentifier($this->collection))
			die(Tools::displayError('Error: 201306051446'));
		...
		/* Database insertion */
		if ($nullValues)
			$result = MongoDbApi::getInstance()->autoExecuteWithNullValues(_DB_PREFIX_.$this->collection, $this->getFields(), 'INSERT');
		else
			$result = MongoDbApi::getInstance()->autoExecute(_DB_PREFIX_.$this->collection, $this->getFields(), 'INSERT');

		if (!$result)
			return false;
		/* Get object id in database */
		$this->id = MongoDbApi::getInstance()->Insert_ID();
	...
	{
	 	if (!Validate::isTableOrIdentifier($this->identifier) OR !Validate::isTableOrIdentifier($this->collection))
			die(Tools::displayError('Error: 201306051448'));
		...
		/* Database update */
		if ($nullValues)
			$result = MongoDbApi::getInstance()->autoExecuteWithNullValues(_DB_PREFIX_.$this->collection, $this->getFields(), 'UPDATE', array($this->identifier => ($this->id)));
		else
			$result = MongoDbApi::getInstance()->autoExecute(_DB_PREFIX_.$this->collection, $this->getFields(), 'UPDATE', array($this->identifier => ($this->id)));
		if (!$result)
			return false;
				...
				foreach ($fields as $field)
				{
					foreach (array_keys($field) as $key)
						if (!Validate::isTableOrIdentifier($key))
							die(Tools::displayError('Error: 201306051449'));

					// used to insert missing lang entries
					$where_lang = '`'.pSQL($this->identifier).'` = '.(int)($this->id).' AND `id_lang` = '.(int)($field['id_lang']);

					$lang_found = MongoDbApi::getInstance()->getValue('SELECT COUNT(*) FROM `'.pSQL(_DB_PREFIX_.$this->collection).'_lang` WHERE '. $where_lang);

					if (!$lang_found)
						$result &= MongoDbApi::getInstance()->AutoExecute(_DB_PREFIX_.$this->collection.'_lang', $field, 'INSERT');
					else
						$result &= MongoDbApi::getInstance()->AutoExecute(_DB_PREFIX_.$this->collection.'_lang', $field, 'UPDATE', $where_lang);
				}
		}
		return $result;
	}
	...
	{
	 	if (!Validate::isTableOrIdentifier($this->identifier) OR !Validate::isTableOrIdentifier($this->collection))
	 		die(Tools::displayError('Error: 201306051450'));

		$this->clearCache();

		/* Database deletion */
		$result = MongoDbApi::getInstance()->Execute('DELETE FROM `'.pSQL(_DB_PREFIX_.$this->collection).'` WHERE `'.pSQL($this->identifier).'` = '.(int)($this->id));
		if (!$result)
			return false;

		/* Database deletion for multilingual fields related to the object */
		if (method_exists($this, 'getTranslationsFieldsChild'))
			MongoDbApi::getInstance()->Execute('DELETE FROM `'.pSQL(_DB_PREFIX_.$this->collection).'_lang` WHERE `'.pSQL($this->identifier).'` = '.(int)($this->id));
		return $result;
	}
	...
	{
		if (!is_array($selection) OR !Validate::isTableOrIdentifier($this->identifier) OR !Validate::isTableOrIdentifier($this->collection))
			die(Tools::displayError('Error: 201306051451'));
		$result = true;
	...
	{
	 	if (!Validate::isTableOrIdentifier($this->identifier) OR !Validate::isTableOrIdentifier($this->collection))
	 		die(Tools::displayError('Error: 201306051452'));

	 	/* Object must have a variable called 'active' */
	 	elseif (!key_exists('active', $this))
	 		die(Tools::displayError('Error: 201306051453'));

	 	/* Update active status on object */
	 	$this->active = (int)(!$this->active);

		/* Change status to active/inactive */
		return MongoDbApi::getInstance()->Execute('
		UPDATE `'.pSQL(_DB_PREFIX_.$this->collection).'`
		SET `active` = !`active`
	...
	protected function getTranslationsFields($fieldsArray)
	{
		/* WARNING : Product do not use this function, so do not forget to report any modification if necessary */
	 	if (!Validate::isTableOrIdentifier($this->identifier))
	 		die(Tools::displayError('Error: 201306051454'));
	...
	protected function makeTranslationFields(&$fields, &$fieldsArray, $id_language)
	{
		...
		foreach ($fieldsArray as $field)
		{
			...
			if (!Validate::isTableOrIdentifier($field))
				die(Tools::displayError('Error: 201306051455'));
	...
	public function validateFields($die = true, $errorReturn = false)
	{
		...
		foreach ($fieldsRequired as $field)
			...
			{
				if ($die) die (Tools::displayError('Error: 201306051456').' ('.get_class($this).' -> '.$field.' is empty)');
		...
		foreach ($this->fieldsSize as $field => $size)
			...
			{
				if ($die) die (Tools::displayError('Error: 201306051457').' ('.get_class($this).' -> '.$field.' Length '.$size.')');
		...
		foreach ($this->fieldsValidate as $field => $method)
			...
			{
				if ($die) die (Tools::displayError('Error: 201306051458').' ('.get_class($this).' -> '.$field.' = '.$this->{$field}.')');
	...
	public function validateFieldsLang($die = true, $errorReturn = false)
	{
		...
		foreach ($this->fieldsRequiredLang as $fieldArray)
		{
			...
			{
				if ($die) die (Tools::displayError('Error: 201306051459').' ('.get_class($this).'->'.$fieldArray.' '.Tools::displayError('is empty for default language.').')');
		...
		foreach ($this->fieldsSizeLang as $fieldArray => $size)
		{
			...
			foreach ($this->{$fieldArray} as $k => $value)
				...
				{
					if ($die) die (Tools::displayError('Error: 201306051500').' ('.get_class($this).'->'.$fieldArray.' '.Tools::displayError('Length').' '.$size.' '.Tools::displayError('for language').')');
	...
	*/
	...
	public function getWebserviceParameters($wsParamsAttributeName = NULL)
	{
		...
		if (!isset($this->{$wsParamsAttributeName}['objectNodeName']))
			$defaultResourceParameters['objectNodeName'] = $this->collection;
		if (!isset($this->{$wsParamsAttributeName}['objectsNodeName']))
			$defaultResourceParameters['objectsNodeName'] = $this->collection.'s';
	...
	{
		$query = '
		SELECT DISTINCT main.`'.$this->identifier.'` FROM `'._DB_PREFIX_.$this->collection.'` AS main
		'.$sql_join.'
		WHERE 1 '.$sql_filter.'
		'.($sql_sort != '' ? $sql_sort : '').'
		'.($sql_limit != '' ? $sql_limit : '').'
		';
		return MongoDbApi::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS($query);
	}

	public function getFieldsRequiredDatabase($all = false)
	{
		return MongoDbApi::getInstance()->ExecuteS('
	...
	public function addFieldsRequiredDatabase($fields)
	{
		...
		if (!MongoDbApi::getInstance()->Execute('DELETE FROM '._DB_PREFIX_.'required_field WHERE object_name = \''.pSQL(get_class($this)).'\''))
			return false;

		foreach ($fields AS $field)
			if (!MongoDbApi::getInstance()->AutoExecute(_DB_PREFIX_.'required_field', array('object_name' => pSQL(get_class($this)), 'field_name' => pSQL($field)), 'INSERT'))
				return false;
		return true;
	}
	...
	{
		if ($all AND isset(self::$_cache[$this->collection]))
			unset(self::$_cache[$this->collection]);
		elseif ($this->id AND isset(self::$_cache[$this->collection][(int)$this->id]))
			unset(self::$_cache[$this->collection][(int)$this->id]);
	}
	...
	public function deleteImage()
	{
		...
		if (file_exists(_PS_TMP_IMG_DIR_.$this->collection.'_'.$this->id.'.'.$this->image_format)
			&& !unlink(_PS_TMP_IMG_DIR_.$this->collection.'_'.$this->id.'.'.$this->image_format))
			return false;
		if (file_exists(_PS_TMP_IMG_DIR_.$this->collection.'_mini_'.$this->id.'.'.$this->image_format)
			&& !unlink(_PS_TMP_IMG_DIR_.$this->collection.'_mini_'.$this->id.'.'.$this->image_format))
			return false;
	...
	public final function beginTransaction() {
		MongoDbApi::getInstance()->executeS('BEGIN');
		return this;
	}

	public final function commitTransaction() {
		MongoDbApi::getInstance()->executeS('COMMIT');
		return this;
	}

	public static final function rollbackTransaction() {
		MongoDbApi::getInstance()->executeS('ROLLBACK');
		return this;
	}
}
