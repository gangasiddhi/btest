...
class HookCore extends ObjectModel
{
	...
	public static function get($hookName)
	{
	 	if (!Validate::isHookName($hookName))
	 		die(Tools::displayError('Invalid hook name'));
	...
	public static function updateOrderStatus($newOrderStatusId, $id_order, $id_order_detail = NULL)
	{
		...
		$return = Module::hookExec('updateOrderStatus', array('newOrderStatus' => $newOS, 'id_order' => (int)($order->id), 'id_order_detail' => $id_order_detail)) AND $return;
		return $return;
	}

	public static function getBankResponseOnOrderStatusChange($newOrderStatusId, $id_order, $amount = Null, $id_order_detail = NULL)
	{
		$order = new Order((int)($id_order));
		$newOS = new OrderState((int)($newOrderStatusId), $order->id_lang);

		$return = Module::hookExec('getBankResponseOnOrderStatusChange', array('newOrderStatus' => $newOS, 'id_order' => (int)($order->id), 'amount' => $amount, 'id_order_detail' => $id_order_detail));
		return $return;
	}
