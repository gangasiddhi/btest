...
var recommend_next_arrow = '{l s='Next' js=1}';
var recommend_previous_arrow = '{l s='Previous' js=1}';
...
{literal}
    ...
    {
        var id_lang= '{/literal}{$cookie->id_lang}{literal}';
        var logged= '{/literal}{$cookie->logged}{literal}';
        var prod_id= '{/literal}{$product->id}{literal}';
        var url = baseDir+'sailthru_recommend_product.php';
        ...
        $.ajax({
                ...
                data : data,
                beforeSend: function(){
                    $('#show_recommend_product').html("<p style='color:#fff;'>loading</p>");
                    $('#show_recommend_product').addClass("loading");
                },
                success : function(res) {
                    $('#show_recommend_product').html(res);
                    $('a#user-interest-btn-scroll-right').attr('title',recommend_next_arrow);
                    $('a#user-interest-btn-scroll-right').text(recommend_next_arrow);
                    $('a#user-interest-btn-scroll-left').attr('title',recommend_previous_arrow);
                    $('a#user-interest-btn-scroll-left').text(recommend_previous_arrow);

                },
                complete:function(){
                    $('#show_recommend_product').removeClass("loading");
                    //init the serialScroll for user-interest recommend products
                    $('#user-interest-thumbs-list').serialScroll({
                        items:'li',
                        prev: 'a#user-interest-btn-scroll-left',
                        next: 'a#user-interest-btn-scroll-right',
                        axis: 'x',
                        onBefore:userInterestSerialScrollFixLock,
                        step: 1,
                        lazy: false,
                        lock: false,
                        cycle:false,
                    ...
                    //alert('here');
                    $('#user-interest-thumbs-list ul').trigger('goto', 1);// SerialScroll Bug on goto 0 ?
                    $('#user-interest-thumbs-list ul').trigger('goto', 0);
                }
        });
    });
{/literal}
...
{*<div id="pb-left-column">*}
    ...
    {foreach from=$categories item=subcategory_arrays name=subcategor_names}
        {assign var=sub_cat_name value=$subcategory_arrays.link_rewrite}
        {if ($breadcrumb_path==$sub_cat_name || $breadcrumb_path_categories==$sub_cat_name )}
            {assign var=sub_cat_link value=$subcategory_arrays.link}
        {/if}
    {/foreach}
    ...
    <div id="bread-crumbs">
        {if $breadcrumb_path != ''}
            ...
            {elseif ($breadcrumb_path == 'dugun-toren-ayakkabı-modelleri' || $breadcrumb_path == 'gelinlikayakkabilari')}
               <a href="{$link->getCategoryLink($bridalshoes_link_rewrite)}" title="{l s='Bridal Shoes'}">{l s='Bridal Shoes'} </a>
            {elseif ($breadcrumb_path == 'bayan-ayakkabi-modelleri' || $breadcrumb_path == 'ayakkabi')}
               <a href="{$link->getCategoryLink($shoe_link_rewrite)}" title="{l s='Shoes'}">{l s='Shoes'} </a>
            {elseif ($breadcrumb_path == 'sandalet-modelleri' || $breadcrumb_path == 'sandals')}
               <a href="{$link->getCategoryLink($sandals_link_rewrite)}" title="{l s='Sandals'}">{l s='Sandals'} </a>
            {elseif $breadcrumb_path == '' || $breadcrumb_path_categories !== ''}
                <a href="{$link->getCategoryLink($shoe_link_rewrite)}" title="{l s='Shoes'}">{l s='Shoes'} </a>
                <span class="bread-crumbs-slash">&nbsp;/</span>&nbsp;
                <a href="{$sub_cat_link}" title="{l s=$breadcrumb_path_categories}">{l s= $breadcrumb_path_categories} </a>
            {/if}
            <span class="bread-crumbs-slash">&nbsp;/</span>&nbsp;
        {/if}
    ...
    <!-- product img-->
        ...
        <div id="views_block" {if isset($images) && count($images) < 2}class="hidden"{/if}>
            ...
            {/if}*}
            <div id="thumbs_list" onclick="$('#medium_image').hide()">
                <ul style="width:{math equation="width * nbImages" width=69 nbImages=$images|@count}px" id="thumbs_list_frame" onclick="$('#medium_image').hide()">
                    {*if isset($images)*}
                        {foreach from=$images item=image name=thumbnails}
                        {assign var=imageIds value="`$product->id`-`$image.id_image`"}
                        <li id="thumbnail_{$image.id_image}" onclick="$('#medium_image').hide()">
                            <a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'large')}" rel="useZoom: 'cloudzoom1', smallImage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'medium')}'" class="{if $jqZoomEnabled}cloud-zoom-gallery{/if}" title="{$image.legend|htmlspecialchars}" onclick="$('#medium_image').hide()">
        ...
        <div id="image-block">
            ...
            {/if}
			<div class="product-page-customer-like-dislike-container">
				<p class="product-page-customer-like-dislike-sub-container">
					<span class="product-page-number-of-likes">{if $productLikesCount.likes > 0}{$productLikesCount.likes}{/if}</span>
					<a class="product-page-customer-like-btn {if $customerLikes} product-like-selected{/if}" productId="{$product->id}"></a>
					<a class="product-page-customer-dislike-btn {if $customerDislikes} product-dislike-selected{/if}" productId="{$product->id}"></a>
				</p>
			</div>
            <a href="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large')}" {if $jqZoomEnabled}class="cloud-zoom" id="cloudzoom1" rel="adjustX:-2, adjustY:-2, position:'inside', zoomWidth:{$mediumSize.width}, zoomHeight:{$mediumSize.height}, showTitle:false" title="{$product->name|escape:'htmlall':'UTF-8'}"{/if}>
                {if $have_image}
                    <img itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'medium')}"
			...
			{if isset($video_link)}
				<div id="medium_image" style="position:relative; top:-490px; left:0; z-index:10000">
					<iframe width="490" height="490" src={$video_link} frameborder="0" allowfullscreen></iframe>
				</div>
			{/if}
	...
	{*User Interest Products List*}
	{*if count($userInterestProducts) >= 4*}
		<div id="user-interest-container">
			<div id="user-interest-heading">
				<h3>{l s='You May Like These'}</h3>
			</div>
			<div id="user-interest-thumbnails-block">
                <div id="show_recommend_product"> </div>
				</div>
		</div>
	{*/if*}
	{*User Interest Products List END*}
    ...
    <div id="fb_comments">
        <fb:comments class="fb-comments" href="{$link->getProductLink($product, $url_product_attribute)}" num_posts="3" width="585"></fb:comments>
    </div>
        ...
        {if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
            <p class="price">
                ...
                {if $priceDisplay >= 0 && $priceDisplay <= 2}
                    <span itemprop="offerDetails" itemscope itemtype="http://data-vocabulary.org/Offer" id="our_price_display" class="our_price"><meta itemprop="currency" content="TRY" /><span itemprop="price">{convertPrice price=$productPrice}</span></span>
    ...
    {*</div>*}{* end of color_size_container *}
        ...
        <form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart.php')}" method="post" {if $color_shoe_combination == 1} onsubmit="return showSizeSelection('{l s='Please select shoe size'}');" {/if} {if $accessory_size_combination == 1} onsubmit="return accessorySizeSelection('{l s='Please select size'}');" {/if}>
            ...
            {if ($product->quantity <= 0 && $url_product_attribute==0) || (isset($color_id_quantity) && $color_id_quantity==0 && $url_product_attribute>0) || ($product->quantity <= 0 && $url_product_attribute>0) || (isset($color_id_quantity) && $color_id_quantity==0 && $url_product_attribute>0)}
                <div itemprop="availability" content="out_of_stock" class="out-of-stock"></div>
        ...
        <div id="newbankoption">
            <img src="{$img_dir}product/payment_details_new2.gif" alt="{l s='NewBankOption image'}" id="new_bank_option_image" />
            {if $color_shoe_combination == 1}
            <img src="{$img_dir}product/shoe_measurement_for_old_product_page.jpg" id="shoe_measurement" alt="{l s='Shoe Size Measurement'}"/>
            {/if}
        </div>
