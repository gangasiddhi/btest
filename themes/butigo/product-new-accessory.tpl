                ...
                {if isset($images)}
					...
					{else}
						...
						{assign var="accessory" value=$accessories[$imagePosition]}
							{if $accessories[$imagePosition].quantity > 0}
...
{* end - changed code *}
		...
		{if isset($accessories) AND count($accessories) > 0}
			<div id="accessory-container">
				<div id="accessory-heading">
					<p>{l s='Product Accessories'}</p>
				</div>
				<div id="accessory-thumbnails-block">
					{if isset($accessories) && count($accessories) > 3}
						<a id="accessory-btn-scroll-left" title="{l s='Previous'}" href="javascript:{ldelim}{rdelim}">{l s='Previous'}</a>
					{/if}
					<div id="accessory-thumbs-list">
						<ul id="accessory-thumbs-list-frame" style="width:{math equation="width * nbImages" width=105 nbImages=$accessories|@count}px">
						{if isset($accessories)}
							{foreach from=$accessories item=accessorie name=thumbnails}
							{assign var=imageIds value="`$accessorie.id_image`"}
							{if $accessorie.quantity > 0}
								<li id="thumbnail_{$accessorie.id_image}" class="accessories-products" productId="{$accessorie.id_product}" productAttributeId="{$accessorie.id_product_attribute}">
									<a href="{$link->getImageLink($accessorie.link_rewrite, $imageIds, 'large')}" rel="useZoom: 'cloudzoom1', smallImage: '{$link->getImageLink($accessorie.link_rewrite, $imageIds, 'medium')}'" class="{if $jqZoomEnabled}cloud-zoom-gallery{/if}" title="{$accessorie.legend|htmlspecialchars}">
										<img id="thumb_{$accessorie.id_image}" src="{$link->getImageLink($accessorie.link_rewrite, $imageIds, 'prodthumb')}" alt="{$accessorie.legend|htmlspecialchars}" height="{$thumbSize.height}" width="{$thumbSize.width}" />
										<p class="accessory-name">{$accessorie.name|truncate:22:'.':true|escape:'htmlall':'UTF-8'}</p>
										<p>{convertPrice price=$accessorie.price}</p>
										<p class="accessory-add-to-cart"><img src="{$img_dir}buttons/accessory-add.png" alt="{l s='Add to Cart'}"/></p>
										<input  style="display:none;" type="radio" name="accessorieProduct" value="{$accessorie.id_product}"/>
									</a>
								</li>
							{/if}
							{/foreach}
					...
					{if isset($accessories) && count($accessories) > 3}
						<a id="accessory-btn-scroll-right" title="{l s='Next'}" href="javascript:{ldelim}{rdelim}">{l s='Next'}</a>
					{/if}
				</div>{*end of views_block*}
			 </div>
			{*
					...
					{foreach from=$images item=image name=thumbnails}
						...
						{else}
							...
							{if $accessory.quantity > 0}
								<li id="thumbnail_{$image.id_image}" class="accessories-products" productId="{$accessory.id_product}" productAttributeId="{$accessory.id_product_attribute}">
										<a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'large')}" rel="useZoom: 'cloudzoom1', smallImage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'medium')}'" class="{if $jqZoomEnabled}cloud-zoom-gallery{/if}" title="{$image.legend|htmlspecialchars}">
											<input type="radio" name="accessorieProduct" value="{$accessory.id_product}"/><label>{$accessory.name|truncate:22:'...':true|escape:'htmlall':'UTF-8'}</label>
										</a>
								</li>
							{/if}
						{/if}
