...
/* <![CDATA[ */
	...
	{ldelim}
		...
		li_content['update'] = '<a href="{$link->getPageLink('address.php', true)}?id_address='+id_address+'&amp;back=order.php?step=2{if $back}&mod={$back}{/if}" title="{l s='Edit'}">{l s='>edit'}</a>';
...
{*if !$opc}
{capture name=path}{l s='address'}{/capture}
...
{/if}
	 <div class="shopping-cart">
		 <div id="address_label">
				<img src="{$img_dir}cart/adress_title.gif" alt="{l s='Shipping Address'}"  width="106" height="16" />
				{*<span>{l s='Shipping address'}</span>*}
		 </div>
		 <div id="adress_display">
			 <form action="{$link->getPageLink('order.php', true)}?step=3" method="post">
				<div class="addresses">
					<p class="address_delivery select" style="display:none;">
					...
					<span>{$address.alias|escape:'htmlall':'UTF-8'}</span>
					{*<p class="checkbox">
		...
		{if !$opc}
			<div class="cart_adress">
				...
				{/if}
				<input type="submit" name="processAddress" value="" class="subm address" id="submitaddress"/>
