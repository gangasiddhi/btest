...
Galleria.addTheme({
    ...
    init: function(options) {
        ...
        // add some elements
        this.addElement('bar','fullscreen','play','popout','thumblink','s1','s2','s3','s4','progress','contents');
        this.append({
            'stage' : 'progress',
            'container': ['bar','tooltip'],
            'bar'   : ['fullscreen','play','popout','thumblink','info','s1','s2','s3','s4','image-nav-bar-left','image-nav-bar-right']
        });
        ...
        var gallery = this,
            ...
            left_nav_bar = this.$('image-nav-bar-left'),
        ...
        //new content
//        left_nav_bar.before("<div style='position: absolute; overflow: hidden;height: 29px;line-height: 29px;left: 390px; right: 295px;bottom: 1px; color: #bbb;'>Contents</div>");
        
            
        this.$('s1').remove();
        this.$('s2').remove();                    
       
       this.$('image-nav-bar-right, image-nav-bar-left,image-nav-right').click(function(){
           OPEN=false;
           thumbs.height( gallery.getStageHeight() ).width( gallery.getStageWidth() ).css('top', OPEN ? 0 : gallery.getStageHeight()+30 );
           //thumb_link['removeClass']('open');
       });
        

//        this.$('image-nav-right').addClass('control');
//        this.$('image-nav-left').addClass('control');
        ...
        if (options.play_link) {
            ...
            play_link.remove();
            //this.$('s1').remove();
            //this.$('image-nav-left').css('left',25);
        }
