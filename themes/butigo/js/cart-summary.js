...
function updateCartSummary(json)
{
	...
	else
	{
		...
		$('.cart_discount').each(function(){
			...
			for (i=0;i<json.discounts.length;i++)
			{
				...
				/*Aplication of credits*/
				if (json.discounts[i].id_discount_type == 4)
				{
						$('.cart_total_delivery').remove();
						$('#credit_discount').html(formatCurrency(-1*json.discounts[i].value_real, currencyFormat, currencySign, currencyBlank));

				}
				/* Discount For Holidays */
				if (json.discounts[i].id_discount_type == 6)
				{

						$('#buy1_get1_discount').html(formatCurrency(-1*json.discounts[i].value_real, currencyFormat, currencySign, currencyBlank));
	...
	if(nbrProducts == 1)
	{
		if('tr.cart_item:not(:has(.last_item))')
			$('tr.cart_item').addClass('last_item');
		$('tr.cart_item').removeClass('first_item');
	}
	/*if(nbrProducts > 0){
            $('#shopping-cart-top a').addClass('check-out');
	}
        if(nbrProducts == 0){
            $('#shopping-cart-top a').removeClass('check-out');
        }*/
