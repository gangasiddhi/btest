...
function updateProductChoiceSelect(id_attribute, id_group)
{
	...
	{
		$('#group_'+id_group+' option[value='+id_attribute+']').prop('selected', 'selected');
...
// Serialscroll exclude option bug ?
function userInterestSerialScrollFixLock(event, targeted, scrolled, items, position)
{
	serialScrollNbImages = $('#user-interest-thumbs-list li:visible').length;
...
function refreshProductImages(id_product_attribute)
{
	...
	userInterestSerialScrollFixLock('', '', '', '', 0);// SerialScroll Bug on goto 0 ?
}

//To do after loading HTML
$(document).ready(function()
{
     img_src = baseDir +'themes/butigo/img/product/new_shoe_size_measurement.jpg';
     $("#shoe_measurement").click( function(){
        $.fancybox('<img src="' + img_src + '"style="margin-left:80px;"/><br /><div id="shoe_measurement_desc"><p>&#149; Ayak ölçünüzü alabilmek için, uzun parmaktan başlayarak topuk sonuna kadar ölçünüzü almanız gerekmektedir.</p> <p>&#149; Bu ölçüyü alabilmek için, ayağınızı düz bir kağıt üzerine koyarak uzun parmağınız ve topuk ucunu çizgi şeklinde belirleyebilirsiniz.</p> <p>&#149; Her iki çizgi arasını cetvel veya mezurayla ölçerek ayak cm inizi öğrenebilirsiniz.</p> <p>&#149; Her iki ayak numarasının farklı olabileceği düşünülerek, diğer ayağınızın da ölçüsünü alarak, büyük ölçüyü tercih etmenizi öneririz.</p></div>', {
        autoSize: true,
        margin: 2,
        padding: 5,
        titleShow: true,
    ...
    $('.accessories-products > *').click(function(e) {
    	var $self = $(this);
        var productId = $self.attr('productid');
        var accessoryProductAttributeId = $self.attr('productattributeid');

        $self.siblings('a').click();

        if (! productId || ! accessoryProductAttributeId) {
        	$self = $self.parents('.accessories-products');
        	productId = $self.attr('productid');
        	accessoryProductAttributeId = $self.attr('productattributeid');
        }

        $('input[name="accessorieProduct"][value=' + productId + ']').prop('checked', true);
        $('input[name="accessoryProductId"]').val(productId);
        $('input[name="accessoryProductAttributeId"]').val(accessoryProductAttributeId);
    });
});
...
function saveCustomization()
{
	...
	customAction = $('#customizationForm').prop('action');
	$('body select[@id^="group_"]').each(function() {
		customAction = customAction.replace(new RegExp(this.id + '=\\d+'), this.id +'='+this.value);
	});
	$('#customizationForm').prop('action', customAction);
