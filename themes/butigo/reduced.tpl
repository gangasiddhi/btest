...
<div class="features_wrapper">
	...
	<h3> {l s='No Discount Products'} </h3>
	<div id="features_header">
		<img alt="{l s='Discount'}" src="{$img_dir}/discount/indirim-butigi-top-image-no-deals.jpg"/>
	</div>
	{/if}
	{if $discount_products > 0}
	{*<div id="features_header">
		<img alt="{l s='Discount'}" src="{$img_dir}discount/indirim-butigi-top-image.jpg"/>
	</div>*}
		<h2>{l s='Discount Boutique'}</h2>
	{if isset($discount_products) AND $discount_products}
	<div id="discount_products" class="container_lookbook">
		{foreach from=$discount_products item=product name=discount_product}
			<div class="showroom_sel_cntnr">
				<div class="showfb_{$product.id_product}">
				<div  class="showroom_sel_shoe {if $product.quantity <= 0}sold_out{elseif ( $product.quantity > 0 && $product.quantity < $last_qties )}low_stock{/if}"
