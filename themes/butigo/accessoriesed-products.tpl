...
<div class="celebrity_wrapper">
	{$HOOK_ACCESSORIESED_PRODUCT_SLIDE_SHOW}

	{if isset($accessoriesedProducts) AND $accessoriesedProducts}
	<div id="all_celebrity_products" class="container_lookbook accessorised-container">
		{foreach from=$accessoriesedProducts item=product name=Celebrity_products key=index}
		...
		{/foreach}
	</div>{*end of all_celebrity_products *}
    ...
    (function(d){ldelim}
		...
		HEIAS_PARAMS.push(['order_article','{foreach name=Celebrity_products item=product from=$accessoriesedProducts}{$product.id_product}{if ! $smarty.foreach.Celebrity_products.last},{/if}{/foreach}']);
